...
4. **整形が効いたかどうか（実機で 40 字以上は 8 件中 0 件）。** 履歴の `refinedText` が
   ...
   （要件定義書 §2.8.4）。**これは V-25 の残りの材料である。**
   **目標 2 秒のほうは 19 / 36 / 56 字の 3 点で既に実測してある**（2026-08-15。傾き約 18.5 ms/字。
   詳細設計書 §10）。**ここで欲しいのは 80 字以上の実測**——3 点だけでは
   121 字 ≒ 2.24 秒という外挿を確かめたことにならない。**打ち切り 3 秒は依然として推定値**で、
   その決め手（利用者が続きを打ち始めるまでの時間）は未実測である（V-29）
...
| **V-37** | **40 字以上の発話**を、**言い終えてから離す**のと**言い終える前に離す**のとで各 5 回 | **書き換わらない発話があるか。** 履歴の `refinedText` が入っているのに欄が生テキストのままなら差し替えの断念、`refinedText` が空なら整形そのものが捨てられている | **言い終えた側で `refinedText` が空になるなら報告する**（整形の検査が厳しすぎる。詳細設計書 §5.5.1 へ）。**言い終える前に離した側が空になるのは設計どおり**——モデルが続きを捏造するので捨てている（要件定義書 §2.8.7）。**生テキストは欄に残る** |
| **V-38** | **数字を含む発話**（「十時から」「百二十パーセント」「一万円」など）を各 3 回。**英語の発話も 3 回** | 履歴の `refinedText` が入っているか。**整形の許容量は「入力に無い文字を 1 字も足さない」まで絞ってある**ので、数字の表記が変わると整形ごと捨てられる（詳細設計書 §5.5.1） | **数字の発話で `refinedText` が空になるなら報告する**——整形プロンプトの規則 5（数字の表記を変えない）が実機で効いていない。**英語で空になるのは既知**（6 発話中 2 が日本語へ訳され、検査が落とす）。**どちらも生テキストは欄に残る** |
...
| **V-36** | 長い発話をして、**書き換わる直前に PTT を押す** | 次の録音の開始が遅れて感じられるか。**代役の欄では最大 3.5 ms（予算 50 ms の 7 %）**だが、**実アプリの AX の往復が 1 回 4 ms を超えると 50 ms を破る**（詳細設計書 §10.1） | 体感できるほど遅いなら、まず `refinementApplyMode` を `beforeInsert` にして消えるか見る。消えるならこの経路が原因で、差し替えを actor の外へ出す判断が要る |
