...
| **V-36** | 長い発話をして、**書き換わる直前に PTT を押す** | 次の録音の開始が遅れて感じられるか。**差し替えは actor の外で走る**ので、AX が遅い相手でも待たされないはず（代役の欄に 1 往復 10 ms を注入して最大 3.1 ms。対処前は 164 ms。詳細設計書 §10.1） | 体感できるほど遅いなら、まず `refinementApplyMode` を `beforeInsert` にして消えるか見る。消えるならこの経路が原因である |
