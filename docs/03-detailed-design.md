...
**観測した範囲では、正当な整形の最大は 2、逸脱の最小は 5。境界は 3 に置いてある。**

> **この境界が分離するのは、上の 13 入力（× 3 回）と逸脱 6 例の集合だけである。
> 普遍的な分類境界ではない。** 実測で分けられなかった側を、次の 2 通りに具体化できる（2026-08-15）。
>
> - **正当なのに落ちる側**: 数量表記の正規化は、桁が増えると 3 字を超える。
>   `参加者は百人です` → `参加者は100人です。` は追加 3（通る）だが、
>   `千円です` → `1,000円です。` は**追加 4 で拒否される。**
>   `十`→`10`（追加 2）を正当と認めるなら、`千`→`1,000` を落とす根拠は無い。
> - **逸脱なのに通る側**: 短い意味の反転は 3 字以内に収まる。
>   `成功` → `失敗。` は**追加 2**、`可` → `不可。` は**追加 1** で、**どちらも境界を通る。**
>
> **どちらも、モデルが実際にその出力を返す頻度は未実測である。**
> 確かなのは「**Guard の実装ロジック上、通る／落ちる**」ことだけで、
> **上の 4 例はレビューが指標を写して構成した反例であり、実 LLM の出力ではない。**
> 過剰要約を捕まえられないことも含め、**最後の安全網は Undo（FR-7）である。**
