...
│   │       ├── HUDIsland.swift        **島の寸法と輪郭。値はすべて決めごと**（§7.3）
│   │       ├── HUDPanel.swift         **NSPanel を作る唯一の場所**（RunLoopEntry が要る）
│   │       ├── HUDContentView.swift   SwiftUI。**動くのは島の矩形 1 つだけ**
...
| **終了要求の猶予切れ**（イベント列の終端） | **「終わらせたい」。この発話をどうするかは言っていない** | **挿入しない。そこまでのテキストを履歴へ `.notInserted` かつ `isProvisional: true` で残す**（下記） |

#### 終了の猶予切れで打ち切った発話を捨てない

**実機 2026-08-15**: 利用者が PTT キーを押したまま喋っている最中に `SIGTERM` が届き、猶予 10 秒を使い切って打ち切られた。**そこまでの発話は欄にもクリップボードにも履歴にも、どこにも残らなかった。** 打ち切りそのものは設計どおりである（無限に待つと終了できないプロセスになる）。**穴は「打ち切ったあとに何もしていない」ことだった。**

**ESC による中断は履歴へ残す**のに、終了の打ち切りだけ穴が空いていた。**同じ「途中でやめる」なのに片側だけである。** `DictationSession.run()` はイベント列が尽きた時点でまだ録音中なら救出する（`salvageAbandonedRecording`）。

- **確定（`finish()`）は撃たない。** 既に猶予を使い切っており、撃つと終了がさらに最大 `finalizeDeadline` 延びる。**その時点でメモリに在るもの**——確定済みの前半（`latestFinal`）＋未確定の末尾（`latestVolatile`）——をそのまま残す
- **したがってこのテキストは確定していない。** `HistoryEntry.isProvisional` を立てる。確定済みと同じ顔で履歴に並ぶと、利用者は「認識がおかしい」と読む（実際には打ち切られただけである）。履歴画面には「暫定（確定前）」のバッジを出す
- **secure input 中は何も残さない**（唯一の例外。基本設計書 §7 / FR-4）。挿入側にだけ例外を置くと、この経路からパスワードが `history.json` へ入る
- **どこへ行ったかを必ず告げる**（`ShutdownSalvage`）。「挿入されませんでした」だけでは、どこにも無いのか履歴にはあるのかが判らず、利用者は次にすることを決められない
- **`isBusy` で行き先を代用してはならない。** 救出は `finishIdle()` まで走るので、**成功直後の `isBusy` は偽**であり、そこだけを見ると打ち切ったことすら告げずに終わる

| 打ち切りの経路 | そのとき発話はどこにあるか | 告げること |
|---|---|---|
| 録音中に猶予切れ（テキストあり・履歴へ書けた） | **履歴**（`.notInserted` / `isProvisional`） | 「暫定テキストを履歴へ残しました」 |
| 録音中に猶予切れ（履歴へ書けない／上限 0） | **どこにも無い** | 「どこにも残せませんでした」＋ `SessionFailure.historyUnavailable(insertedElsewhere: false)` |
| 録音中に猶予切れ（secure input 中） | **どこにも無い。意図した結果である** | 「パスワード入力中だったため残していません」 |
| 録音中に猶予切れ（一言も認識されていない） | 失うものが無い | **何も告げない** |
| 確定〜挿入の最中（`.processing`） | **`run()` が `completionTask` を見届ける**ので通常どおり完走する | 打ち切りではないので何も告げない |
| 待機中 | 抱えていない | 何も告げない |

> **猶予 10 秒を延ばして守ろうとしてはならない。** 延ばしても「終わらないプロセス」に近づくだけで、押しっぱなしの利用者は必ず追い越す（実機では 10 秒を丸ごと使い切った）。**守りは打ち切りの側に置く。** `Shutdown.defaultGrace` は**決めごとであり実測ではない**（`Shutdown.defaultGrace` の doc コメントに明記）。
...
public protocol AudioCapturing: AnyObject, Sendable {
    ...
    /// **寝ていれば自分で起こす**（実測 中央値 67.3 ms）
    func startTap(format: AVAudioFormat?) throws -> AsyncStream<AVAudioPCMBuffer>
    /// タップを外す。エンジンは止めない
    func stopTap()
    /// エンジンを止める。**マイクのオレンジ点が消えるのはここだけ。**
    /// タップが張られている間は何もしない。冪等
    func sleep()
    /// エンジンが動いているか。**タップの有無とは別の軸**
...
- `AVAudioEngine` は `prepare()` で**捨て起動を 1 往復（`start` → 即 `stop`）してから止める**
  （2026-08-16 改訂。旧: 「`start()` まで済ませ、以後停止しない」）。
  以後、`startTap` が寝ていれば自分で起こす。**「起こす」口は公開しない**——
  起こし忘れを型として作れなくするため。いつ止めるかは `DictationSession` の方針
  （既定 30 秒。`defaultAudioIdleSleepDelay`）。常時起動をやめた理由は
  `coreaudiod` の常時 +15 ポイントであり、**マイクのオレンジ点を点けているのは
  タップではなくエンジンそのものである**（設計書 2026-08-15 §2）。
- 録音の開始／停止は `inputNode.installTap` / `removeTap` のみで行う
  （エンジンの起床・停止はこれとは別の軸である）。
...
最悪値は計算上 1282 ms**（§10 の留保 2。M2 199 + 打ち切り 750 + ⌘V 33 + 復元待ち 300）で、
**V-3 の実アプリ実測でも `total` 1182〜1447 ms と 1000 ms を割り込んでいる**（要件定義書 §2.8.5 (1)）。
**Pasteboard 経路の M5a を縮めたくなったら、まずここを読むこと。**
...
#### **デーモンは使わないと冷える。起動時に 1 度暖めるだけでは足りない**（実測 / 2026-08-15）

上の「常駐はデーモン側にある」は**片側しか書いていなかった。**
**プロセスを作り直しても速いまま**である一方、**同じプロセスの中でも間隔が空けば冷える。**

同じ 9 字の発話（`お勧めで大丈夫です`）を、間隔を変えて実 LLM へ通した値
（`FoundationModelRefiner.generate` の実時間 / MacBook Pro Mac15,3 / M3 / macOS 26.5.2）:
...
**1 発目だけが遅く、直後の 2 発目は必ず 330〜490 ms へ戻る**（＝1 回きりの再ロード費用であって、
プロセスが遅くなったのではない）。**眠らずに 60 秒待った対照でも 909 ms** だったので、
**`Task.sleep` による休止（App Nap）でもない。**

**実運用の発話間隔は 6 秒〜2.5 分**（利用者の履歴 / 2026-08-15）。
**つまりほぼ毎回この費用を払っており、(b) の予算 750 ms は再ロードだけで尽きる**
（要件定義書 §2.8.9）。

**対処は `DictationSession.warmRefinerForUtterance()`**——**録音の開始（PTT 押下）で暖める。**
押下から解放までの間に暖機を終えれば、解放の時点ではデーモンが温まっている。
**投げっぱなしにする**ので「押しても何も起きない時間」は作らない（§4.1 の 9 / 基本設計書 §6）。
**直前に推論を投げていれば投げ直さない**（`refinerWarmthWindow`。既定 10 秒。
**要件値ではない**——連射のときに暖機が本番の前へ割り込むのを避けるためだけの値）。
**起動時の捨て推論も同じ印を立てる**（立てないと、起動直後に押した発話が
捨て推論の後ろへもう 1 本暖機を積む。実運用でその形の失敗が 1 件あった）。

**効くことを実測した**（同じ 9 字の発話 / 60 秒の間隔を置いた後 / `lead` は押下から解放までの時間）:

| `lead`（発話の長さ） | 暖機なし（現状の 1 発目） | **録音開始で暖機** |
|---|---|---|
| 0.5 秒 | 955 ms | **571 ms** |
| 1.0 秒 | 866 ms | **414 ms** |
| 2.0 秒 | 1095 ms | **402 ms** |
| 3.0 秒 | 1239 ms | **448 ms** |

**0.5 秒しか喋らなくても 750 ms の内側に入る。** 1 秒以上喋れば温まりきった値
（400 ms 台）へ落ちる。**実運用の発話は 1 秒より長い**ので、この差がそのまま効く。

> **これは代役ではなく実 LLM の値だが、実アプリ・実マイクでは未確認である**（V-45）。
> 実機では認識器（`SpeechAnalyzer`）が同時に走るので、暖機と競合しうる。
...
言語名は認識ロケール（`Settings.localeIdentifier`）から決める（`en` なら `English`、それ以外は `日本語`）。ja-JP の指示のまま
英語を渡すと、実測で「会議は明日の午前10時に開催されます。」と**日本語へ訳された**。
（それでも英語 6 発話中 2 発話は日本語へ訳される。**検査が落とすので生テキストが挿入される。**
規則を足して直せる見込みが無いため、現状は縮退で受けている。）

#### 規則 5 は効いた——**効く規則と効かない規則の違いも実測から言える**（2026-08-15）

**規則 5 は `RefinementGuard` の成立条件である**（§5.5.1）。数量表記の正規化（`十` → `10`）は
それ自体が逸脱ではないため、起きる限り検査で許容するしかなく、**許容した量はそのまま逸脱にも開く。**
ここで凍結すれば、数字の変化は「起きてはならないこと」になり、**検査は追加を 1 字も許さない形にできる。**

| 入力 | 規則 5 なし | 規則 5 あり（各 3 回同一） |
|---|---|---|
| `…会議は十時から…` | `…10時から…`（追加 2） | `…十時から…`（追加 0） |
| `予算は一万円です` | `予算は1万円です。`（追加 1） | `予算は一万円です。`（追加 0） |
| `…前年比で百二十パーセント…` | `…120％…`（**追加 3 = 当時の許容量ちょうど**） | `…百二十パーセント…`（追加 0） |

数字を含む 12 発話のうち応答が返った 10 発話すべてで**追加 0 字**になり、
数字を含まない 36 発話の出力は規則 5 の有無で変わらなかった（回帰なし）。

**規則 4（要約するな）と規則 6（続きを補うな）が効かなかったのに、規則 5 が効いた理由は
「表層の形」か「意味」かの違いである。** 前者 2 つはモデルに**意味の判断**を求める。
...
| **内容が 1 字も残らない出力でない** | 追加字数の検査は出力の内容文字を数えるので、`。。。` のような出力は 0 字と数えて素通りする |
| **入力に無い語の追加が 0 字** | **整形は消す操作と、句読点・空白・大文字小文字を変える操作しかしない。語を足したらそれは整形ではない**（下記） |
...
**コードフェンスの検査は、今や単独では効かない**（最終レビューの変異検査 G6 / G6b が
生き残ったことで判明した）。バッククォートは Unicode の分類が Sk（記号）で「自由に足してよい文字」に
入らないため、**フェンス 1 個で追加 3 字となり、許容量 0 の検査が必ず落とす。**
旧設計書はここに「入力と同程度の短いコード片は長さの検査を素通りする」と書いていたが、
**その素通りの受け手はコードフェンスではなく追加字数の検査だった。**

残してある理由は 2 つ。(1) 追加字数は共通部分列を取るので O(入力 × 出力) 掛かり、
暴走した生成をそこへ渡す前に切る早期打ち切りとして効く。(2) **入力側にフェンスがある場合だけは
独立に効く**——そのときフェンスは入力由来なので追加 0 字となり、追加字数の検査は通してしまう。
音声認識はバッククォートを返さないが、**FR-6 の辞書は利用者が手で書く**。
「整形結果にコードフェンスは決して現れない」は入力に依らない絶対条件として残す。
...
出力のうち、**入力にも句読点にも由来しない文字の数**（共通部分列を取り、句読点・空白は
両側から除き、大文字小文字を畳んでから数える）。

**句読点を勘定に入れないのが要点である。** 整形の仕事の一つが句読点の補完なので、
入れると**節の多い長い発話ほど落ちる**指標になり、V-37 が疑った長さ依存を直した側で作り込む
（実測: 句読点の少ない 49 字の発話で読点 2 + 句点 1 を足す）。
**句読点の項を外したので、句読点の数に比例して増える成分は無くなった。**
**ただし「発話長に依存しない」とまでは言えない**——長い発話ほど数字・略語・表記正規化の
機会そのものが増えれば、発話全体での追加字数は増えうる。**この点は未実測である**
（測ったのは 5〜124 字の 13 入力で、長さと追加字数の関係を直接見てはいない）。

**大文字小文字を畳むのは英語のため。** 実測（en-US 6 発話）で
`so the meeting is at ten…` → `The meeting is at ten…`、
`you know the build is failing…` → `Build is failing…` と、**大文字化だけで追加 1 字**が付いた。
畳まないと、許容量 0 の検査が**英語の整形を丸ごと落とす。** 畳めば 6 発話とも追加 0 字になる。

#### 許容量は 3 字ではなく 0 字である（V-38 / 2026-08-15 に作り直した）

**当初の許容量は 3 字だった**（「正当側の最大 2 と逸脱側の最小 5 の間隙」に置いた）。
**この間隙は 13 入力の標本の中にしか無かった。** 最終レビューの反証（`review-6-refute.md` §5）を
受けて標本を 71 入力へ広げると、**正当な数量正規化が境界ちょうどの 3 字に達していた**——
`…前年比で百二十パーセント` → `…前年比で120％`。余裕は 0 で、`千二百` → `1,200` なら 4 字で落ちる。

**問題は閾値の位置ではない。** 数量表記の正規化は**それ自体は逸脱ではない**から許容するしかなく、
**許容した量はそのまま逸脱にも開く。** 閾値をどこへ動かしてもこの二律背反は動かない。
**指標をいじる直し方は、これで 2 回続けて破れた。**

**直した形は「曖昧な類を起こさせない」である。** 整形プロンプトへ**規則 5「数字の表記を変えない」**を
足した（§5.4）。実測で効き、数量表記の追加は全部 0 字になった。曖昧な類が消えたので、
検査は閾値ではなく**性質**になる:

> **整形結果は、句読点・空白・大文字小文字を除けば、入力の部分列でなければならない。**

**製品のプロンプトそのもので 71 発話（日英・1〜76 字）を実測**した結果
（2026-08-15 / MacBook Pro M3 / macOS 26.5.2 / temperature 0）。70 発話が応答し、61 が受理された:
...
| **逸脱: 質問への回答** | 2 | **6** |
| **逸脱: 続きの捏造** | 3 | **7 / 10 / 38** |
| **逸脱: 日本語への翻訳**（en-US 6 発話中 2） | 2 | **14 / 15** |
| **逸脱: 依頼への回答**（前置き・コード片） | 2 | **128 / 166** |

**受理側は 61/61 が 0 字。逸脱側の最小は 6 字。1〜5 字は 1 例も無い。**
**0 は標本から引いた線ではない。「整形は足す操作ではない」という定義であり、標本が増えても動かない。**

**代償を正しく理解しておくこと。** 許容量 0 は、モデルが 1 字でも入力に無い文字を足せば整形を捨てる。
**捨てたときに起きるのは生テキストの挿入**（利用者が言ったことは失われない）であり、
**逸脱を通すより軽い。** この非対称が、線を厳しい側へ倒す根拠である。

**この値を上げるなら §5.4 の規則 5 とセットで考えること。** 規則 5 を外すと数量正規化が復活し、
追加 1〜3 字の**正当な**出力が生まれて整形が落ち始める。**プロンプトと検査は一体で設計してある。**

**用語の正規化（FR-6）を逸脱と分ける工夫は残してある。**
`ジーエイエスを使いました` → `Google Apps Script を使いました。` は素で測ると **16 字の追加**で、
許容量 0 字はもちろん、逸脱の最小 6 字も大きく超える。だから**頼んだ置換も由来として認めてから**測る
（`permittingVocabulary`）。認めた後は追加 0 字になる。**辞書を渡さなければ同じ出力が落ちる。**

##### **置き換えてはならない。両方を残す**（実運用の欠陥 / 2026-08-15）

当初この関数は誤認識表記を正規表記へ**置き換えて**いた。**その形は「置換は必須である」と
言っているに等しく、実機で整形を丸ごと落とした。**

利用者が辞書へ `頭, 列, スレッズ → Threads` を入れた状態で、**モデルが置換をしなかった**とき:

```
raw               : 頭のAPIが使えるようになったので
置き換えた expected : ThreadsのAPIが使えるようになったので
モデルの出力        : 頭のAPIが使えるようになったので      ← 入力と 1 字も違わない
判定               : 追加 1 字（`頭`）で**拒否**
```

**`頭` は利用者が実際に言った（認識された）文字なのに、`expected` から消えていたので
「入力に無い文字」に数えられた。辞書に語を 1 つ入れた瞬間、置換されなかった発話の整形が
全部落ちる**——**FR-6 が FR-5 を壊していた。**

**置換は「してもよい」であって「しなければならない」ではない。**
誤認識表記を消さず、**その直後に正規表記を挿し込む**（`頭` → `頭Threads`）。
出力がどちらの表記を採っても追加 0 字になり、**利用者が言っていない語だけが追加として残る。**
**許容量 0 は動かしていない。緩めたのは「何を由来とみなすか」だけである。**
回帰は `辞書があってもモデルが置換しなければ、入力そのままの出力を受け入れる` が押さえている。

> **辞書には別の実測もある（直していない。要件定義書 §2.8.1 L-6 の延長）。**
> 極端に短い発話へ辞書ブロックを付けると、モデルが整形ではなく**辞書についての注釈**を返す:
> `レッツ` → `レッツは、Threads、API、Slack、Notion、Xcodeのいずれかです。`（追加 34 字）、
> `す` → `すは、スと発音するので、スと表記してください。`（追加 19 字）。
> **検査が落とすので生テキストが挿入される**が、**短い発話では辞書が整形の成功率を下げる。**
...
- **語の途中で切れた場合の補完（`…ありがとうござい` → `…ありがとうございます。`）も落ちる**（追加 2 字）。
  許容量 3 の頃はこれを通していたが、**0 では通らない。** 失うのはその発話の整形だけで、
  生テキストは挿入される。**意味の捏造と語形の補完を字数で分けることはできない**という
  反証の指摘（`review-6-refute.md` §5）を、**分けようとしない形で受け入れた。**
- **削ることで意味が変わる逸脱は原理的に捕まえられない。** この検査が見ているのは
  **足された文字**だけである。
  ...
  **境目は「意味が変わったか」ではなく「入力に無い文字を使ったか」である。**
  L-5（過剰要約）が落ちるのも意味を見たからではなく、たまたま語尾を書き換えていたからにすぎない。
  **文字の編集距離を測る指標は意味を見ていないので、指標の改良では解けない。**
  この限界を狭めるには**意味を見る手段**が要る。**現状その手段は無い。**
- **安全網は 2 つ。どちらも実装済みである。**
  **FR-7（Undo）**（差し替えできる経路で挿入した発話に限る）と、
  **履歴が `rawText` を保持していること**（`HistoryEntry.rawText`。整形後だけでなく
  **生テキストが必ず残る**ので、利用者は言ったことを確認・回収できる）。
- **短い意味反転は実機で一度も観測されていない**（`可` `不可` `承認` `却下` `中止` `続行` を含む
  18 の短い発話で、モデルは反転しなかった）。反証が挙げた `成功` → `失敗。` は手で構成した出力で、
  **実際に `成功` を投げると 52.1 秒暴走して `exceededContextWindowSize` になる**（応答なし＝縮退）。
...
**読み戻しを「一致するか」だけで判定しない理由。** 書き込み後に読み戻して「入っていない」と判定したとき、それが本当に失敗なのか、アプリがテキストを変換・整形しただけなのかを区別できない。誤って失敗と判定するとフォールバックが走り、**AX で入ったテキストの上にもう一度貼り付ける。** 二重に入るのは、入らないことより悪い場合がある（ユーザーが気づかずに送信する）。**この裁定は今も生きている**——下の検知は「一致しない＝失敗」ではない。

#### 無言失敗（R-4）の検知（**2026-08-15 に実観測されたので入れた**）

**観測（実機 / macOS 26.5.2 / M3 / Google Chrome 139）。** Chrome のページ内にある素の `<input>` は、
`role=AXTextField`・`kAXSelectedText` は `settable=yes` と答え、`AXUIElementSetAttributeValue` は
`.success`(0) を返す。**それでも欄には 1 文字も入らない。**

```
書込前の選択: loc=8 len=0 → 書込後の選択: loc=8 len=0   ← キャレットが動かない
文字数(kAXNumberOfCharacters): 8 → 8                    ← 1 文字も増えていない
[caret,0] の読み取り → 読めた（長さ 0）                   ← 読み戻しには応える
[caret,5] の読み取り → unreadable(-25212)                ← 書いたはずの場所に何も無い
```

**利用者の実害として現れた。** 物件情報のアンケートフォーム（素の `<input>`）へ 2 回喋って
2 回とも入らず、履歴には 2 件とも `ax`（成功）として残っていた（2026-08-15 21:02:11 / 21:02:16）。
**§11.3 の表で「Chrome ページ内は `AXWebArea` で書込不可 → Pasteboard」と記録していたのは
ページ本体にフォーカスがある場合で、フォーム欄にキャレットを置くと別の要素（`AXTextField`）が
返る。**——AX 経路が選ばれるのはこちらである。

**検知の規則。** 次の 3 つがそろったときだけ「入っていない」と断定し、`.failed` を返して
二段目（⌘V）へ落とす。**1 つでも欠ければ従来どおり成功として扱う。**

| # | 条件 | なぜ要るか |
|---|---|---|
| 1 | 書き込みの前後で**キャレットが位置も長さも完全に一致** | 何かが起きた相手を除く。**これ単独では断定しない**——書き込みに成功しつつキャレットを書き込み位置の先頭へ戻す相手と区別が付かない |
| 2 | 相手が**範囲の読み戻しに応える**（`AXStringForRange`） | 応えない相手では「入っていない」ことを確かめる手段が無い。**判定は長さ 0 の問い合わせで行うので、中身を 1 文字も明かさない**（NFR-V3） |
| 3 | **書いたはずの場所に、書いたはずの文字列が無い** | 1 の除外。範囲の単位は未実測（V-23）なので `utf16` / `Character` / `unicodeScalars` の 3 通りを試し、**どれか 1 つでも一致すれば「入っている」側へ倒す** |

**縮退の向きは「従来と同じ」であって、二重挿入ではない。** 判らない相手（読み戻しに応えない・
キャレットが動いた）は成功として扱うので、この検知が誤って二段目を撃つことはない。
**費用を払うのは失敗した発話だけである**——ふつうに入る相手は条件 1 で抜けるため、
読み戻しの往復は 1 度も増えない（実測: Chrome アドレスバーは 3/3 で `ax` のまま・錨も取れる）。

**残っている限界。** 二段目（⌘V）は錨を持てないので、**この経路へ落ちた発話には整形が反映されない**
（生テキストのまま残り、`.refinementNotApplied(nil)` を告げる）。Chrome のように
「`kAXSelectedTextRange` は settable なのに書き込みが効かない」相手では `canCaptureAnchor()` が
真を返すため、(a) の分岐に載ってから錨が取れない形になる。**発話は失われない**が、整形は届かない。
消すには「一度無言失敗した pid では以後 AX を試さない」（C-7 と同じ形）を足す必要があり、
**その場合は同じ pid のアドレスバーも道連れで Pasteboard へ落ちる。** 未実施。
...
try? await Task.sleep(for: restoreDelay)   // 貼り付け完了を待つ。既定は PasteboardInserter.defaultRestoreDelay
restore(saved, to: pb)
```

| 項目 | 設計判断 |
|---|---|
| 復元待ち時間 | **値の正本は `PasteboardInserter.defaultRestoreDelay`（現在 300 ms）。** 短すぎると貼付前に復元してしまう。**フェーズ 1 の 120 ms は V-3 で不足と判明し、300 ms へ引き上げた**（下記） |
...
**この節は「測った値」と「採った値」を分けて書く。** 測った値は変わらないが、採った値は
実機の観測で 120 ms → 300 ms へ動いた。混ぜて書いていたために、値を変えた後も
この節だけが 120 ms のまま残った（フェーズ 2 の最終レビュー #1）。

##### 測った値（2026-08-13 / MacBook Pro M3 / macOS 26.5.2 / 自プロセスの `NSTextView`）
...
つまり実測 35 ms は**下限**であり、上限ではない。

##### 採った値（現在 300 ms。**`PasteboardInserter.defaultRestoreDelay` が正本**）

**フェーズ 1 は 120 ms を採った**（実測 35 ms の約 3.4 倍）。**V-3（2026-08-14 / 実機）で
この値は不足と判明したので、300 ms へ引き上げた。** 上の留保 2 がそのまま現実になっている——
Electron 製の端末アプリ（cmux）へ挿入すると、**相手が ⌘V を処理する前に復元が走り、
戻された「前のクリップボードの内容」が貼られた。** 履歴には正しい発話が `pasteboard` で
成功として記録されるので、**「成功と記録されるのに、そこに違うテキストがある」**——
このプロジェクトが繰り返し潰してきた形である。連続発話で悪化した（前の発話の復元が、
次の発話の書き込みと ⌘V の間へ割り込むため）。`--paste-restore-delay-ms 300` で解消した。
経緯の全文は `PasteboardInserter.defaultRestoreDelay` の doc コメントと要件定義書 §2.8.5 (3) にある。

**300 ms が上限である保証は無い。** 確かめたのは「120 では壊れ、300 では壊れない」までで、
**間の値も、より重いアプリも測っていない**（未実測）。壊れ方がデータの取り違えなので、
速さより余裕を優先している。

> **この節の値を下げようとする人へ。** 120 ms は「据え置いてよい値」ではなく
> **実機で事故を起こして棄却された値**である。下げる前に V-3 を実アプリで測り直すこと。

なおこの待ち時間は NFR-P5（テキスト挿入 50 ms 以内）には数えない。挿入はテキストが貼り付いた時点で完了しており、復元はその後始末である。**ただし `Metrics` の M4 は `insert(_:)` の実時間を測るので、この待ちが丸ごと乗る**（実測: 復元待ち 120 ms のとき M4 は 170 ms、300 ms のとき 324〜342 ms）。§10 の留保 2 を参照。
...
>
> **置けても嘘になる窓がもう 1 つある。** Pasteboard 経路は「退避 → ⌘V → 復元待ち（既定 300 ms）→ 復元」の間クリップボードを握っており、**その待ちのあいだ `DictationSession` の actor は解放されている。** 次の発話がその窓に入っている最中に Undo キーが届くと、縮退が置いた生テキストを**復元が上書きする**（最終レビュー 視点1 の B-2）。**世代（`InsertionEpoch`）では塞げない**——直列化するのは AX の書き込みで、クリップボードはそこを通らない。`DictationSession` 側に「挿入がクリップボードを握っている」印を持ち、その間は `.undoUnavailable` へ倒す。**発話は履歴にあるので、告げ間違えるより告げない方を取る。**
>
> **この 2 つは差し替え器を本番へ配線するまで到達不能だった。** `clipboard` が nil のあいだ、縮退は必ず `.undoUnavailable` で戻っていた。
...
> **この表は「結末の種類」を並べたものであって、実行経路を列挙したものではない。**
> 実装の終端 `return` は 18 箇所あり、それを 6 行（`.declined` の内訳まで数えて 17 の
> 結果カテゴリ）へ畳んでいる——たとえば「フォーカスが無い」と「別の要素にフォーカスがある」は
> どちらも `.declined(.focusChanged)` に落ちるので 1 行になる。
> **したがって「表にある＝すべての中止点を尽くした」ではない。**
>
> **表に独立した行を持たない結末が 1 つある**: **手順 6（選択範囲の書き戻し）の失敗。**
> `setSelectedRange` の戻り値は捨てているので、**復元に失敗しても結果は `.replaced` /
> `.silentlyIgnored` / `.declined(.textWriteFailed)` のいずれかのまま**である。
> **テキストは正しいが、利用者のカレット・選択が差し替え範囲に残る**——次に打った文字の
> 入る位置が変わりうる。**発話は失われないので重さは低いが、表からは読み取れない。**
> **実アプリでの発生は未実測**（代役の AX は手順 3 と手順 6 を独立に失敗させられない）。
...
> → **採った形**: FR-2 の「notch 部分に表示する」は、**切り欠きを覆う 1 枚の黒い島**にした（§7.3。2026-08-15 に帯から作り直した）。
> **島は切り欠きの周囲（左右と下）を黒く塗る**ので、
> **切り欠きの中に画素があれば繋がって塗られ、無ければ物理的に黒いだけ**である——
> **どちらでも「1 枚の黒い面」に見える。したがって V-20 の結果で実装は変わらない。**
> 中身（文字・音量バー）は帯より下にしか置かないので、画素が無くても読めなくならない。
> **目視の手順は [引き渡し手順書](04-handover-verification.md) の「段 7: HUD の目視」にある。**
...
| `ignoresMouseEvents` | `true` | クリックも奪わない。**島がメニューバーの帯の一部を覆うので、ここが偽だと隠れた項目が押せなくなる**（§7.3） |
| `backgroundColor` | **`.clear` + `isOpaque = false`** | **窓は島より大きい**（`HUDIslandMetrics.maximumSize`）。島の外はメニューバーが透けて見えなければならない |
| `animationBehavior` | **`.none`** | 既定（`NSPanel` は `.utilityWindow`）だと order-in のたびに**縮小＋淡入**が入る。**実測でその途中の窓は `y=1` / `511x104`**——島が画面の一番上から 1 pt 離れ、「浮いた板」に見える（§7.3 の実測） |
...
**2026-08-15 に、隠すことを承知の上で島の形へ作り直した**（§7.3）。以前の形——切り欠きと同じ幅の帯が下へ伸びる——は、
利用者の言葉で「**部分的に何か突き出して大きくなっている感じ**」であり、**1 枚の面には見えなかった。**
島は切り欠きの左右へ食み出すので、**帯の一部を必ず覆う。** 引き換えに得たものと、覆う量の上限は §7.3 に書く。
...
- `CGEventTapHotkeyMonitor` はソースを `CFRunLoopGetMain()` の `.commonModes` へ足す。**`NSApp.run()` の下でも `.commonModes` のソースが処理されることは実測した**（2026-08-14 / M3 / macOS 26.5.2）。**タップと完全に同じ形**（`CFMachPort` 由来の version 1 ソース）を含む 4 系統、`run()` の**前後どちらで登録しても**、**メニュー追跡中・モーダル中・`LSUIElement` の `.app` バンドル内**でも発火した。**陰性対照**（`.defaultMode` にだけ足したソース）は同条件で 1 度も発火しないので、偽陽性ではない。配送遅延は p50 0.045 ms（裾は 5〜17 ms）。
  - **成立範囲の但し書き 0（もっとも重い）**: **実測したのは「同じ形のランループソースが処理される」ことだけである。`CGEvent.tapCreate` は一度も呼んでいない**（入力監視の権限ダイアログを誘発するため）。**実キーイベントがタップ経由で配送されるか・`return nil` による ESC の抑止が効くか・`.tapDisabledByTimeout` が出ないかは、いずれも未実測である**（V-19）。**ここが外れると PTT がまったく動かない。** ランループの実測は「疑いの大半を消した」のであって、「タップが動くことを示した」のではない。
...
#### 実バンドルで窓が出ることの実測（2026-08-15 / Mac15,3 / M3 / macOS 26.5.2）

利用者の実機で「HUD がまったく出ない」という申告が出たため、**署名済みの `.app` を `/Applications` から起動して外側から観測した。**

| 測ったこと | 実測値 |
|---|---|
| `CGWindowListCopyWindowInfo` に現れる HUD の窓 | `layer 26 / alpha 1.0 / onscreen true / bounds (791, 0, 221, 78)`（CG 座標＝左上原点）。**これは帯だった頃の値である。島にした後は `641, 0, 521x106`**（同日の再測。§7.3） |
| 切り欠きの帯（画面上端 100 pt）に掛かる窓の z 順 | メニューバー本体 **24**（Window Server） < コントロールセンターの各項目 **25** < **HUD 26**。**26 より前面に居る窓は無かった**（`Accessibility Services` の 3002 は alpha 0.0） |
| `NSPanel` 側から見た姿 | `isVisible true / alphaValue 1.0 / level 26 / occlusionState に .visible / screen (0,0,1800,1169)` |
| 中身が塗られているか | `contentView` を `cacheDisplay` して不透明画素を数え、標本 4329 点中 **4316 点が非透明** |

**したがって「描画・座標・level・透明度・サイズ」はいずれも実バンドルで成立する。** `--hud-check` で出ない場合、原因はこの 4 つではない。

#### 「出したつもり」と「本当に出た」を区別する 1 行を製品に置いた

上の申告では、**窓は作られていた（`[HUD] 表示先:` が出ていた）のに、録音中に一度も order-in されていなかった。**
それを示す手がかりがログに 1 行も無かったため、実機のログからは切り分けられなかった。

- `HUDPanel.render` は**表示の出し入れの瞬間だけ** `[HUD] 窓を出しました/引っ込めました: 矩形 … / level … / 可視 … / 画面 …` を出す。**暫定テキストの更新回数では増えない**（`.hidden` ↔ 可視の変わり目でしか呼ばない）。**発話は 1 文字も含めない**（FR-12 / NFR-V2）。
- `NotchHUDSurface.subscribe` は購読の成立を `[HUD] 状態の購読を開始しました（購読者 N）` として出し、**成立していなければその旨を言う。** 分配器は登録済みの購読者にしか配らない（`SessionBroadcast.yield`）ので、ここが 0 のままだと録音しても HUD は永久に何も映さない。
- 状態の購読が**終端した**ときも黙って抜けさせない（`[HUD] 状態の購読が終わりました。以後 HUD は何も映しません。`）。`SessionBroadcast` は終端後に購読すると即座に終端したものを返すので、**一度も回らずに抜ける形もありうる。**

#### `--hud-check` は製品と同じ経路を通る

**以前の素振りは `HUDPanel.render` を直に叩いていた。** つまり `HUDPresenter` も `NotchHUDSurface.handle(_:)` も購読も 1 行も通らず、
**「素振りは出るのに録音では出ない」という形の欠陥を、この確認は原理的に捕まえられなかった。**

いまは `HUDRehearsal.wiringScript`（`HUDEvent` の列）を **`handle(_:)` へ入れて**から、見た目を網羅する `HUDRehearsal.script` を回す。
既定の秒数は 2 つの筋書きの総和（約 15.4 秒）を上回るよう **20 秒**にしてある（`HUDRehearsalTests` が総和と比べて固定）。
**島にしたときに筋書きを 2 手増やした**（行数の上限を超える長さの暫定テキスト／広げた島から畳んだ島へ戻るところ）ので、
総和が 12.4 秒へ伸び、既定も 16 秒から引き上げた。**「広がるところ」と「縮むところ」が両方あることは検査で固定してある**
（`HUDRehearsalTests.scriptShowsTheIslandGrowAndShrink`）——片道だけだと、利用者は形が変わることを一度も見ない。
...
#### 実装した形（**島**。2026-08-15 に帯から作り直した）

##### なぜ作り直したか

利用者の申告（原文）:

> ストリーミングの部分の**背景が全体的に黒くなっていない**ので、**部分的に何か突き出して大きくなっている**感じがしています。
> 何かされてるところが見えないとか**1 行分しか見えない**感じになってるので、
> **ダイナミックアイランドみたいな感じでフローティングみたいな感じで全体的に出る** UI/UX になるとより良いかな。

**以前の形は切り欠きと同じ幅（実測 221 pt）の帯が下へ 40 pt 伸びるものだった**（窓 `791,1091 221x78`）。
切り欠きの左右を透明にしていたので、**黒い面は切り欠きの幅しか無く、「帯が突き出している」ようにしか見えなかった。**
**挙動には問題が無い**という評価だったので、**変えたのは見た目だけである。**
...
```
        ...
        ↓
  ─────╮                       ╭─────   ← 逆アール（半径 12。メニューバーの帯へ滑らかに繋ぐ）
       │  ┌───────────┐        │
       │  │  切り欠き  │        │        ← **切り欠きは島の黒の中に完全に埋まる**
       │  └───────────┘        │
       │   音量 言語 暫定文字   │        ← 中身はすべて帯より下（最大 3 行）
       ╰───────────────────────╯        ← 角丸（半径 22）
```

##### **V-20（切り欠きに画素があるか）が未実測のままでよい理由は変わっていない**——ただし理由は入れ替わった

- **以前**: 中身をすべて帯より下へ置いたので、切り欠きが真っ黒でも中身は読める（帯の黒は飾りだった）。
- **いま**: **島は切り欠きの周囲（左右と下）を黒く塗る。**
  - 画素が**ある**なら、切り欠きの中も黒く塗られて周囲と繋がる。
  - 画素が**無い**なら、切り欠きの中は物理的に黒いだけ——**周囲が黒いので同じに見える。**
  - **どちらでも 1 枚の黒い面になる。** 中身は帯より下にしか置かないので、画素が無くても読めなくならない。

**したがって V-20 の結果で実装は変わらない。** 目視は「どちらだったか」を記録するためだけに行う。

##### 引き換えにしたもの（**メニューバーを一部覆う。承知の上の判断**）

**島は切り欠きの左右へ食み出すので、メニューバーの帯を必ず一部覆う。**

| | 値 |
|---|---|
| 覆う幅（畳んだ島） | **301 pt**（x 751..1052。切り欠きの左右へ画面の上辺で 40 pt ずつ、逆アールを過ぎた一番細いところで **28 pt** ずつ） |
| 覆う幅（広げた島） | **520 pt**（x 641.5..1161.5） |
| 画面の幅（実測） | 1800 pt。**左に 641 pt・右に 638 pt のメニューバーが残る** |

- **クリックは奪わない**（`ignoresMouseEvents = true`）。**隠れている項目もそのまま押せる。**
- **出ているのは発話のあいだだけ**である（`.idle` では `orderOut`）。
- メニュー項目は左から、ステータス項目は右から並ぶので、**時計もコントロールセンターもアプリのメニューも通常はここに来ない。**
  **ただし項目が非常に多いアプリでは重なりうる**（実機の目視項目に入れた。段 7）。

##### 寸法と角丸は**すべて決めごとである**（実測値と混ぜないこと）

正本は `Sources/GhostVoiceApp/Shell/HUD/HUDIsland.swift` の `HUDIslandMetrics`。
検査 `HUDIslandTests.theDecidedNumbersAreFixed` が 1 つ残らず固定している。

| 何を | 値 | 決めごと / 実測 |
|---|---|---|
| 切り欠きの矩形・帯の高さ | x 791 / y 1131 / w 221 / h 38 | **実測**（`HUDScreenSnapshot` が画面から取る） |
| 切り欠きの左右へ食み出す量 | 40 pt | **決めごと** |
| 畳んだ島の幅 | `max(300, 切り欠き + 80)` = 実測の切り欠きで **301 pt** | **決めごと** |
| 広げた島の幅 | `max(520, 畳んだ幅)` = **520 pt** | **決めごと** |
| 帯より下の高さ（畳んだ / 広げた） | **44 / 68 pt** | **決めごと** |
| 暫定テキストの行数の上限 | **3 行** | **決めごと**（以前は 2 行） |
| 画面の上辺へ繋ぐ**逆アール**の半径 | **12 pt** | **決めごと** |
| 下側の角丸の半径 | **22 pt** | **決めごと** |
| 浮かべるとき（notch 非搭載機）の上側の角丸 | **18 pt** | **決めごと** |
| 形が変わる時間 | **0.22 秒** | **決めごと**（要件値ではない。§7.4） |
| 色 | **黒**（`Color.black`） | **決めごと** |

> **切り欠きそのものの角丸半径は実測していない。**
> **島は切り欠きを覆うので、見えている角はすべて島が描いたものである**——
> したがって切り欠きの半径と一致させる必要が無い（一致させようがない）。
> 「角丸が notch の曲線と連続して見えるか」は**目視項目**である（段 7）。

##### 窓は島より大きく、**表示が変わっても動かない**

| | 値 |
|---|---|
| 窓（実測の切り欠きで） | **`641.5, 1063, 520x106`**（＝広げた島がちょうど収まる大きさ） |
| 畳んだ島 | 301 × 82 |
| 広げた島 | 520 × 106 |

**窓は透明でクリックも透かす**ので、島より大きくても見た目にも操作にも出ない。
代わりに **`setFrame` が表示の変わり目で 1 度も走らなくなる**——動くのは SwiftUI の中の島の矩形 1 つだけになる（§7.4）。
窓が動くのは**画面構成が変わったときだけ**である（`relocate`）。

**実バンドルでの実測（2026-08-15 / Mac15,3 / M3 / macOS 26.5.2 / `--hud-check=16`）**:
`CGWindowListCopyWindowInfo` に現れた Ghost Voice の窓は **`641,0 521x106 / layer 26 / alpha 1.0` の 1 つだけ**だった
（CG 座標＝左上原点。**y=0 なので画面の一番上に接している**）。
**`animationBehavior = .none` を入れる前は、これに加えて `646,1 511x104 / alpha 0.0005` と `642,0 519x106` が現れた**——
`NSPanel` 既定の縮小＋淡入の途中の姿であり、**`y=1`（画面の上辺から 1 pt 離れる）**が含まれていた。

##### notch 非搭載機（V-41。**未実測**）

帯が無いので、**四隅とも丸い島をメニューバーの直下に浮かべる**（逆アールは描かない）。
幅は同じ決めごと（畳んだ 300 / 広げた 520）、高さは帯のぶんだけ低い（44 / 68）。
...
#### 島にした後のやり直し（実測 2026-08-15 / 署名済み `.app` / MacBook Pro Mac15,3 / M3 / macOS 26.5.2）

**窓が大きくなり（221x78 → 520x106）、メニューバーの帯を覆うようになったので、同じ観測をやり直した。**
`open -n` で新しいインスタンスとして `--hud-check=16` を起動し、外側から 0.25 秒ごとに観測した。

```
=== 起動前（3 秒 / 12 回） ===  frontmost(layer0) pid=85997 name=Google Chrome
                                最前面(layer0) が Ghost Voice だった回数: 0 / 12
=== 素振り中（19 秒 / 74 回） ===  frontmost(layer0) pid=85997 name=Google Chrome
                                  GhostVoice の窓の layer=[26]（出ている間）/ []（畳んでいる間）
                                  最前面(layer0) が Ghost Voice だった回数: 0 / 74
                                  Ghost Voice の窓の矩形（重複を除く）: 641,0 521x106 layer 26 alpha 1.0
=== 終了後 ===                    残存プロセス 0（利用者のインスタンスだけが残った）
```

- **最前面 pid は 1 度も Ghost Voice にならなかった**（0 / 86）。**挿入先の判定は壊れていない。**
- **layer は 26 のまま。** 窓を大きくしても z 順は変わらない（緑ドット 2147483630 より前には出ない）。
- **矩形は 1 つしか現れなかった。** `animationBehavior = .none` を入れる前は淡入の途中の姿（`646,1 511x104 / alpha 0.0005` など）が混ざっていた。

> **観測の道具の落とし穴**: 最初の観測器は最前面のアプリ名に `"Ghost"` が含まれるかで数えていた。
> **端末の `Ghostty` が引っかかって「51 / 51 奪われた」と出た。** 名前の部分一致では数えられない——**pid で数えること。**
...
|---|---|
| `idle` | 非表示 |
| `recording` | 音量バー（`AudioCapturing.level` に連動）＋ 言語バッジ（日/EN）＋ 暫定テキスト（**末尾 3 行**、`.volatile` 更新ごとに差し替え） |
...
| エラー | メッセージを 3 秒表示 |
| **終了待ち** | **「終了待ち: PTT キーを離してください（残り N 秒）」を 1 秒ごとに更新して出す。** 状態の並びに由来しないので `HUDPresenter.announceShutdown` で直接置く（`GhostVoiceCore.ShutdownAnnouncement`） |

#### 終了待ちを HUD に出す（2026-08-15 の実機欠陥）

終了の段取りは正しく動き、正しい文言を出していた。**しかし `.app` では unified log にしか出ず、画面には何も出なかった。** 利用者は PTT キーを押したまま「全然反応しません」と言い、案内を一度も見ないまま猶予 10 秒を使い切った。**正しく待っているのに壊れて見えていた。**

決めたこと（**どれも決めごとであり実測ではない**）:

| 何を | どうするか | なぜ |
|---|---|---|
| いつ出すか | **待ち始めの `.waiting` では出さない。1 秒後の「まだ待っています」から出す** | 待機中に終了要求が来ると 0.13 秒で終わる（実測 V-34）。そこで出すと一瞬光って消えるだけで、不安しか残さない |
| 残り時間 | **1 秒ごとに更新する**（`Shutdown.heartbeat`） | **残りが動いていること自体が「固まっていない」の証拠**である。静止した文言では「待っている」と「死んでいる」を区別できない——直前に直した欠陥がまさにそれだった |
| `.idle` のとき | **「`.idle` なら非表示」の規則より終了待ちを優先する** | 規則の目的は「発話が無いときに邪魔をしない」ことであり、終了待ちは発話の有無に関わらず**利用者の行動（キーを離す）を待っている** |
| 録音中の更新 | **譲らない**（`HUDPresenter.shutdownUntil`）。`.recording` の暫定テキストにも上書きされない | 終了待ちがいちばん要るのは「押しっぱなしで喋っている最中」であり、そこでは `.recording` が 50 ms ごとに届く。ふつうの告知だと一瞬で消える |
| 色 | **失敗として出さない**（`.actionRequired` → `.info`） | 終了待ちは失敗ではない。赤く出すと「壊れた」と読まれる——**それがこの欠陥の症状そのものである** |
| フォーカス | **奪わない。** 通るのは `HUDPanel.render` の `orderFrontRegardless()` だけ | 利用者は終了待ちの間も挿入先アプリで作業している |
| ログ | **HUD の有無にかかわらず必ず出す**（`AppShutdownAnnouncer.sink` はログ → HUD の順） | **HUD が死んでいる状況でも終了待ちは起きる。** 直前の欠陥は「メインキューが詰まって `@MainActor` が全部死ぬ」形で、そのときログだけが残った |

**目視は `--hud-check` で行う**（素振りの一覧に「終了待ち」があり、最後に製品と同じ経路で 1 度出す）。

#### `Duration` を利用者向けの文字列にするときは `JapaneseDuration` を通す

実機のログに `[終了] 10.0 seconds 待っても待機へ戻りませんでした。` が出た（2026-08-15）。`"\(Duration.seconds(10))"` は `10.0 seconds` である。**他がすべて日本語なので、ここだけが英語で浮く。** 変換は `Sources/GhostVoiceCore/Support/JapaneseDuration.swift` の 1 箇所に置き、文言の側では素の補間を禁じる（`ShutdownSequenceTests.announcementsAreJapanese` が固定）。
...
##### 島にしたときに増やしたアニメーションと、それを予算に収めた形（2026-08-15）

**ダイナミックアイランドの要点は形が変わることである**（利用者の要望）。しかし上の規律は「継続アニメーションを置かない」であり、**足し方を間違えると PTT の反応が鈍る。** 収めた形は 3 つ:

1. **動くのは矩形 1 つだけ。** SwiftUI の `.animation(_:value:)` が見るのは **`islandSize` だけ**である。**暫定テキストが変わっただけでは何も動かない**——動く材料は `HUDDisplay` の中身ではなく大きさに限ってある。
2. **大きさは 2 通りしか取らない**（畳んだ島 / 広げた島。`HUDDisplay.wantsWideLayout`）。**発話の長さで連続に変えない**ので、1 発話でアニメーションが走るのは数回である。行数に上限（3 行）を置いたのも同じ理由——上限が無いと高さが `.volatile` の更新ごとに動く。
3. **窓は動かさない。** 窓は最大の島が収まる固定寸法で、**`setFrame` は表示の変わり目で 1 度も走らない**（§7.3）。**帯だった頃より窓の仕事は減っている。**

**0.22 秒という長さは決めごとであり、要件値ではない**（`HUDIslandMetrics.expansionSeconds`。検査 `HUDIslandTests.expansionIsShort` は 0.5 秒未満であることだけを見ており、**その 0.5 秒も要件値ではない**）。

**間接的な観測（2026-08-15 / 実バンドル / `--hud-check=16`）**: 素振りの各手はログに時刻付きで出る。台本の秒数との差は**どの手でも +18〜67 ms** に収まり、**形が変わる手（島 301→520）と変わらない手のあいだに差は見られなかった**（+47 ms 対 +63〜67 ms）。
**これは `CGEventTap` の配送を測ったものではない。** V-39（HUD が PTT の反応を鈍らせていないか）は**依然として未実測**である。
...
public struct HistoryEntry: Codable, Sendable, Identifiable {
    ...
    // .ax / .pasteboard / .clipboardOnly / .notInserted（ESC で中断された発話・終了で打ち切った発話）
    public let isProvisional: Bool       // 既定 false。**終了の猶予切れで打ち切った発話だけ true**（§4）
                                         // 復元は decodeIfPresent。**鍵が無い古い history.json も読める**
                                         //（1 つ足りないだけでファイル全体の復元に失敗すると、履歴が更新で消える）
}
...
- **`append` が返すのは「実際に残ったか」であって「例外が出なかったか」ではない。** 上限 0（設定画面のステッパーで到達でき、`normalized` が「明示的な指示として尊重する」と定めたサポート構成）では、`append` は何も保存せず例外も投げない。2 値で扱っていた頃は、**ESC で中断した発話が欄・クリップボード・履歴のどこにも残らないまま、失敗を 1 つも出さずに待機へ落ちていた**（フェーズ 2 の最終レビュー A-1）。`DictationSession.record` は 3 値（残った / 残らなかった / 書けなかった）で受け、**中断経路は「残った」以外を失敗として扱う。**
- **保存ファイルの権限を明示する（ディレクトリ 0700 / ファイル 0600）。** umask 任せにしていた頃、実機の `history.json` は 0644 だった。**いま安全なのは親ディレクトリ（`~/Library/Application Support`）が 700 だからであって、このコードが守っているからではない**——保存先は `init(rootURL:)` で差し替えられるので、`~/Library` の外へ置いた瞬間に保護が消える（フェーズ 2 の最終レビュー P-3）。`history.json` には認識テキストが平文で入る。**`.atomic` 書き込みは実体を毎回差し替えるので、権限も毎回付け直す。**
- **「履歴を全部消す」は退避（`history.json.corrupt`）も消す。** 退避の中身は**利用者の発話そのもの**である。`history.json` を空配列で書き直すだけだった頃は、**利用者が「全部削除」を押しても過去の発話がディスクに残り続けた**（フェーズ 2 の最終レビュー P-2）。「削除した」と「まだ在る」が食い違ってはならない。**書き直しの後に消す**——先に消すと、書き直しが失敗したときに「退避だけ消えて履歴は残る」状態になる。
...
#### 手順（5 手 + 後始末。**中止したときは「何も書き換えていない」状態に戻る**）
...
1. **利用者は 4 つの権限を `Ghost Voice` へ付け直すことになる。** これは避けられない。
   移行手順は [引き渡し手順書](04-handover-verification.md) の「1. 最初にやること: 権限の付け直し」にある。
...
| Apple Development 証明書 | `identifier "com.haruki1090.GhostVoice" and anchor apple generic and certificate leaf[subject.CN] = "…" and certificate 1[…]` | **DR は 1 文字も変わらなかった**（実測。同一ソースで 2 回、実コードを変えて 1 回、戻して 2 回の計 5 回のビルドで確認） |

> **この実測が言えることの範囲（2026-08-15 に狭めた）。**
> 5 回のビルドは**すべて同じ機体・同じ Xcode 26.6・同じ Apple Development identity・
> 同じ `CFBundleIdentifier`** で行っている。**変えなかったものが同じなら DR も同じ**、というのが
> 実測の内容である。「証明書署名なら何をしても DR は変わらない」ではない。
>
> **DR の式そのものが、変わる条件を明示している**——`identifier "…"` と
> `certificate leaf[subject.CN] = "…"` を文字列として含むので、
> **バンドル ID を変えれば変わり、別の CN の証明書で署名し直せば変わる。**
>
> **未実測**: 証明書の更新・失効・再発行（CN が同じでも別レコードになりうる）、
> Xcode の更新、`CFBundleIdentifier` 以外の `Info.plist` 変更。
> **したがって「再ビルドしても許可を付け直さずに済む」と断言できるのは、
> 上の 4 条件を 1 つも動かさない再ビルドに限る。**
> **そもそも「DR が同じなら TCC の許可が実際に残る」こと自体が未実測である（V-16）。**
...
| `M1c` | **スリープからの起床 → タップ武装**（アイドル 30 秒で止めた後の最初の 1 回） | **実測 中央値 67.3 ms / 最小 58.9 / 最大 87.0**（2026-08-16 / M3 / macOS 26.5.2 / n=20・実 HAL。`wakeLatency`）。**改訂後の NFR-P1 はこれを例外として許容する**（連続する発話は M1a のまま 50 ms 以内）。**コールドの初回 `start()` 214.7 ms とは別の量**であり、`prepare()` の 327〜456 ms とも別物である。コールドぶんは起動時の捨て起動で払う。**M1b と違い、これは配達の遅れではなく本当の欠落である**——エンジンが止まっている間の音は存在しない（設計書 2026-08-15 §2.4） |
...
| `M4` | **挿入器の呼び出し → 復帰**（(a) では生テキスト、(b) では整形結果） | 50 ms（NFR-P5）。**実測 中央値 0 ms（両条件）。ただしこれは `.clipboardOnly` 経路に固定した計測で、⌘V の往復（33 ms）も復元待ち（既定 300 ms）も含まない**（下記の留保 3 件） |
...
>
> （**この 1 文はその後の実測で古くなった。** 逸脱を止めているのは
> **`unsupportedAdditions`（入力に無い文字の追加が 0 字）** である。長さ比が拾うのは
> 大きく膨らんだ逸脱だけで、コードフェンスの検査は追加字数の検査に包含される。§5.5.1）
>
...
> NFR-P6b（目標 2 秒 / 打ち切り 3 秒。**目標 2 秒は 19〜56 字の実測で裏づけた（2026-08-15）。
> 80 字以上は外挿【推測】のまま。打ち切り 3 秒は推定値で、V-29 が残っている**）で走る。
>
...
#### 判ったこと 1: **(a) は M5a を 1 桁下げる。ただし「発話長に依存しない」とまでは言えない**

同じ発話・同じ機体で **(b) 393〜828 ms に対し (a) 38〜75 ms。**

**(a) の M5a は発話長に対する伸びが小さい。しかし「依存しない」ではない。**
測った 3 点は**単調に増えている**（低負荷 45 → 54 → 75 ms、負荷下 38 → 45 → 73 ms）。
**19 字から 56 字で 30〜35 ms 増えており、3 点へ直線を当てた傾きは 0.8〜1.0 ms/字である。**
整形（当時の値で約 18.5 ms/字。**この値は下の「判ったこと 2」の追記で否定された。温まったデーモンでは 5.26 ms/字**）に比べれば小さい傾きだが、ゼロではない。
増分の出どころは M2（確定が出そろうまで）で、**M2 自体が発話長とともに伸びる**（44 → 51 → 72 ms）。

**正しい言い方**: 「整形がクリティカルパスから外れると、**NFR-P6a の発話長への依存が
整形の傾き（当時 18.5 ms/字、測り直して 5.26 ms/字）から M2 の傾き（約 1 ms/字）へ落ちる**」。
「依存しなくなる」は測った 3 点自身に反する。

**測っていないこと**: 80 字・121 字、肉声、実マイク、実アプリの欄。
**この 3 点は 1 本の合成音声を 3/6/10 秒で切ったもので、挿入先は代役の欄（`FakeTextField`）である。**
実アプリの M4（AX 12〜34 ms / Pasteboard 307〜354 ms）はこの値に乗っていない。達成の確定は V-28。
...
**この 3 点に当てた傾きは約 18.5 ms/字、切片はほぼ 0。** 要件定義書 NFR-P6b が
「出力長にほぼ比例する【推測】」として置いていた外挿（121 字 ≒ 2.4 秒）は、
**この 3 点と矛盾しなかった**（18.5 × 121 ≒ 2.24 秒）。

> **【この直線は 2026-08-15 の実運用の切り分けで否定された。要件定義書 §2.8.9 が正である。】**
>
> **3 点は、間に 3〜10 秒の実時間の音声再生を挟んで測っている。**
> **モデルのデーモンはその間に冷える**ので、**各点が「冷え直しの費用」を丸ごと含んでいた**
> （§5.2 の実測: 間隔が空いた 1 発目は 0.7〜2.1 秒、直後の 2 発目は 0.33〜0.49 秒）。
> **傾きの大部分は発話長ではなく間隔だった。**
>
> **温まったデーモンで測り直した値**（利用者の実発話 9 種 × 6 回 / 1〜161 字 /
> 連続投入 / 2026-08-15 / MacBook Pro Mac15,3 / M3 / macOS 26.5.2）:
>
> | 字数 | 1 | 5 | 9 | 17 | 18 | 46 | 98 | 161 |
> |---|---|---|---|---|---|---|---|---|
> | 中央値 | 443 | 362 | 408 | 403/449 | 474 | 641 | 810 | 1250 |
>
> **9 点に当てた直線は 365 ms + 5.26 ms/字。**
> **750 ms を割るのは 40 字ではなく約 73 字である。**
> 121 字の外挿も 2.24 秒ではなく **約 1.0 秒**になる。
> **「18.5 ms/字」を引用しないこと。**

> **【推測が実測に置き換わったのは 19〜56 字の範囲だけである。】**
> 121 字は**測っていない**。**56 字より先で飽和する曲線も、加速する曲線も、
> 同じ 3 点を通せる。** したがって「121 字 ≒ 2.24 秒」は依然として**外挿＝推測**であり、
> 実測ではない。**80 字以上を 1 点でも測るまで、この数字を実測として引用してはならない**
> （V-25 の残り）。
>
> **測定条件**: 1 本の合成音声（`say -v Kyoko`）を 3 / 6 / 10 秒で切ったもの。
> 各点 n=5 × 2 負荷条件 / 2026-08-15 / MacBook Pro Mac15,3 / M3 / macOS 26.5.2。
> **56 字の点は文の途中で切れており、モデルが 40 字を捏造する特殊な入力だった**（判ったこと 3）。
> **3 点のうち 1 点がこの性質を持つことは、線形性の根拠としては弱い。**
...
| 選択肢 | 何が起きるか |
|---|---|
| **上げる（例 1000 ms）** | (b) の予算は `M2 199 + T + M4`。AX 経路の (b) でも 199 + 1000 + 34 = **1233 ms** で NFR-P6a を破る |
| **下げる（例 450 ms）** | 当時の観測では、実運用に近い 36 字の帯（M3 620〜752 ms）が丸ごと打ち切られる |
| **据え置き 750 ms** | AX 経路の (b) は 199 + 750 + 34 = **983 ms ≦ 1000** |

> **【この論証は 2026-08-15 の追試で 3 つとも足元を失った。据え置きの結論は「積極的に選んだ値」ではなく
> 「比較していないので動かさない」に格下げする。】**
>
> 1. **「上げても 56 字は Guard に落ちるので見返りが無い」は成り立たない。**
>    56 字の 10/10 却下は**旧 Guard・文の途中で切れたフィクスチャ**での結果だった。
>    **新しい Guard では、同じ長さ帯の言い終えた発話は 3/3 で反映される**（§5.5.1 の対照実験）。
>    しかも (a) の 56 字は M3 969〜1067 ms で終わっており、**1000 ms なら拾える標本が実在する。**
> 2. **「下げると 36 字の帯を失う」の根拠も揺れた。** 当時「36 字は 9/10 反映」と数えたものは、
>    **+11 字の捏造を受け入れていた回**である（判ったこと 3 の追記）。
>    **新しい Guard の下で 450 / 750 の各値が正当な整形を何件通すかは測り直していない。**
> 3. **「Pasteboard は 450 でも達成しない」は、報告値からは導けない。**
>    根拠にした `total` 1182〜1447 ms は**復元待ち 300 ms を含む値**であり、
>    要件が問うのは「テキストが現れる時点」である。差し引くと 882〜1147 ms。
>    さらに、その標本が打ち切りに張り付いていた回なら、750 → 450 で表示時点はさらに前へ動く。
>    **個々の標本が打ち切り律速だったかの内訳が無いので、判断がつかない。**
>
> **したがって「750 が最適」とは言えない。言えるのは「450 / 750 / 1000 を新しい Guard で
> 直接比較していないので、いま動かす根拠が無い」までである**（比較実験は未実施）。
> **動かすときは、3 値を新 Guard で並べて測ること。**
...
> **【「(b) は狭い」は実運用では成り立たなかった（実測 2026-08-15 / 要件定義書 §2.8.9）。】**
> 利用者の履歴 10 件のうち **9 件が pasteboard 経路**である
> （`canCaptureAnchor()` が偽になるターミナル・ブラウザ等）。
> **(b) はこの利用者にとって既定の経路であり、例外ではない。**
> **「(a) が既定だから (b) の予算は多少苦しくてよい」という論は使えない。**
>
> **750 ms は今回も動かしていない。動かす必要が無かった**——(b) が落ちていた主因は
> 発話長ではなく**モデルの冷え**で、**押下の時点で暖める**ようにすれば
> **温まった状態の 365 + 5.26×字 で走り、73 字までは 750 ms に収まる**（§5.2）。
> **3 値（450 / 750 / 1000）の比較実験は依然として未実施である。**

### M4 について（3 つの留保）

**1. 計測は `.clipboardOnly` 経路に固定してある。** AX 判定を「適用外」に、送出器を
「送出不可」に差し替えて測っている。**Task 8 が実測した ⌘V → 貼付の p50 33 ms も、
クリップボードの復元待ち（既定 300 ms）も含まれていない。** 権限のある機体での確定は
...
`inserter.insert(_:)` の実時間を測るので、**Pasteboard 経路を通った場合は復元待ち
（既定 300 ms）が M4 に入る。**

したがって Pasteboard 経路の最悪ケースでは、`insert()` の復帰までで
**M2 199 + 打ち切り 750 + (⌘V 往復 33 + 復元待ち 300) = 1282 ms** となり NFR-P6a を超える。ただし
**ユーザーにテキストが見えるのは復元待ちの前**であり、体感の M5a は **982 ms** である。
一段目の AX 経路（往復 0.1〜5.5 ms）にはこの待ちが無い。

> **この計算は 2026-08-15 に引き直した。** 旧版は M2 を旧定義の 177 ms、復元待ちを
> フェーズ 1 の 120 ms で組んでおり、`177 + 750 + (33 + 120) = 1080 ms / 体感 960 ms` としていた。
> **M2 は V-12 の修正で 199 ms へ、復元待ちは V-3 の事故で 300 ms へ動いている。**
> 数字を差し替えても**結論（Pasteboard 経路は NFR-P6a を超えうる）は変わらない**が、
> 超過の幅は 80 ms から 282 ms へ広がった。
>
> **この 1282 ms は実測ではなく、別々に測った最悪値を足した上限**である
> （M2 199 ms 自体が「別々の計測の最悪値の和」。同時に起こることは未確認）。
> **実アプリでの端から端の実測は V-3 の `total` 1182〜1447 ms**（要件定義書 §2.8.5 (1)）で、
> こちらは実測である。
...
**この節は 2 度測っている。** 1 度目（差し替えを actor 上で同期に走らせていた実装）で
「配線ぶんは予算を食わない」と結論したが、**その結論は往復の速い相手でしか成り立たない**
ことが最終レビューで指摘され、対処したうえで測り直した。**以下は対処の前後の実測である。**

**計測環境**: MacBook Pro Mac15,3 / M3 / macOS 26.5.2 / 2026-08-15 / 各条件 n=5。
...
#### 対処前（差し替えを actor 上で同期に走らせていた）

| AX 1 往復の注入 | 押下 → 録音開始（低負荷） | 押下 → 録音開始（負荷下 `yes` 16 本） |
|---|---|---|
| **0 ms（代役の欄）** | 中央値 1.6 ms / 最大 **1.9 ms** | 中央値 2.4 ms / 最大 **2.5 ms** |
| 2 ms | 中央値 38.6 ms / 最大 **39.1 ms** | 中央値 37.6 ms / 最大 **37.9 ms** |
| 10 ms | 中央値 155.4 ms / 最大 **164.2 ms** | 中央値 151.3 ms / 最大 **170.0 ms** |

**待たされる量はほぼ `12 × AX 1 往復`。** AX の往復の上限は 1 回 0.5 秒
（`SystemAccessibility.messagingTimeout`）なので、**固まった相手では最大約 6 秒
actor が塞がる。** その間 `run()` は `for await event in hotkey.events` から再開できず、
**PTT の押下も解放も、届いているのに処理されない**——利用者は喋っているのに録音が
始まらず、**発話が丸ごと落ちる。**

しかも `beginPendingRevision` は意図的に `.idle` へ戻して次の PTT を受け付ける設計なので、
**「差し替え中に次の発話が来る」は例外ではなく通常経路**である。

> **1 度目の判断（「対処は行わない」）が誤っていた理由。** 当時の根拠は
> 「測れた側のコスト（≦ 3.5 ms）は予算を食っていない」だったが、
> **測ったのは往復が µs で返る代役だけ**であり、**遅い相手の場合が見落とされていた。**
> 上の 2 ms / 10 ms の行は当時も取れていたのに、結論には 0 ms の行しか使っていない。

#### 対処後（差し替えは actor を手放して走る）

| AX 1 往復の注入 | 押下 → 録音開始（低負荷） | 押下 → 録音開始（負荷下 `yes` 16 本） |
|---|---|---|
| **0 ms（代役の欄）** | 中央値 2.4 ms / 最大 **2.7 ms** | 中央値 2.5 ms / 最大 **2.6 ms** |
| 2 ms | 中央値 1.1 ms / 最大 **2.0 ms** | 中央値 2.1 ms / 最大 **2.2 ms** |
| 10 ms | 中央値 2.2 ms / 最大 **3.1 ms** | 中央値 2.6 ms / 最大 **3.0 ms** |

**待たされる量が AX の往復コストに依存しなくなった。** 往復が 0.5 秒の相手でも
押下は待たされない（12 × 0.5 = 6 秒 → 3 ms 程度）。
**注入 0 ms では前後で変わらない**（1.9 → 2.7 ms / 2.5 → 2.6 ms。どちらも予算の 5 % 台）。

#### 対処の中身と、安全性をどう保ったか

1. **`replacer.replace` / `undo` は `DictationSession.runOffActor(_:)` を通す。**
   `Task.detached` で actor を手放してから同期の AX 往復を走らせる。
2. **`performUndo` は `run()` のループから待たずに起こす**（`beginUndo`）。
   ループの中で `await` すると、actor を手放していてもループ自体が止まり、
   その間の PTT が処理されない。
3. **挿入との重なりは世代の錠が塞ぐ**（`InsertionEpoch.withExclusiveWrite`）。
   1 度目に「actor の外へ出せない」とした理由は
   「世代の照合と実際の書き込みの間に窓ができる」ことだったが、
   **その窓は actor ではなく専用の錠で閉じられる。**
   世代（`InsertionEpoch`）は挿入器と差し替え器が**必ず同じ組で受け取る**もの
   （`InsertionStack`）なので、**新しい共有物を配線しなくて済む。**

**残る未知は「実アプリの AX 1 往復のコスト」だが、判断には要らなくなった**——
待たされる量が往復コストに依存しないためである。
往復コスト自体は挿入の所要（NFR-P6a）には効くので、V-28 で測ることは変わらない。
...
| `DictationSession` | 代役を差し込んで PTT 1 回ぶんの流れを検査する。**発話が落ちうる箇所を名指しで押さえること**: `begin()` → タップの順序、`stopTap()` の末尾が `finish()` より前に供給されること、確定が**結果ストリームの終端**で進むこと（`finish()` の復帰ではない）、**解放後に確定が 2 件届いても 2 件目を取りこぼさないこと（V-12。実音声では駆動できないので代役で決定的に駆動する）**、録音中の確定で進まないこと、ストリームが終端しなくても締め切りで抜けること、確定が来なくても暫定テキストへ縮退すること、**起動時の捨て往復が畳まれてから次の `begin()` が始まること**、secure input が整形の手前で効くこと、中断でも履歴に残ること、最大録音時間で抜けること、キャンセル文脈でも確定処理が完走すること。**フェーズ 2 で追加**: 差し替え可能と判定した発話で**整形を待たずに生テキストが挿入されること**、保留中の差し替えが**次の PTT を妨げないこと**、次の発話の挿入で保留中の差し替えが**破棄され、かつ何も書き換えられていないこと**、**保留が 2 件重なっても新しい方を ESC / Undo キーで取りやめられること**（整形の返る順番を検査から握って決定的に駆動する。`GatedRefiner`）、**挿入がクリップボードを握っている間は FR-7 の縮退がクリップボードへ置かないこと**（挿入が返らない代役で窓を作る。`GatedInserter`） |
| **`TextReplacer`（§8.3）** | **代役の AX で結果カテゴリを 1 つずつ通す**（**「全ての実行経路」ではない**。§8.3 の注記。18 の終端 `return` を 17 カテゴリへ畳んでおり、検査はカテゴリ内の既知の分岐を駆動する。**カテゴリの網羅性そのものは検査していない**）: 事前条件が欠ける / 事前検査が不一致 / 範囲設定が `AXError` / 書き込みが `AXError` / 事後検査が source のまま（無言失敗）/ 事後検査が第三の値（R-9）。**どの場合も ⌫ と ⌘V の追撃が 0 回であること**、R-9 でだけクリップボードへ退避して以後そのプロセスを締め出すこと、**成功時に履歴の同じ `id` が更新されること**。**Undo は同じ関数を逆向きに呼ぶだけであることを、検査でも 1 つの対象として扱う** |
...
**実施の手順は [04-handover-verification.md](04-handover-verification.md) の「段 4」にある**。
...
**2026-08-14 に利用者が一部を実施したので、判った行だけを埋めてある**（要件定義書 §2.8.5 / §13 の V-3）。
**空欄は「異常なし」ではなく「未実施」である。埋めるのは権限を付与した人である。**

| アプリ | 想定される経路 | 結果 | 復元待ちは足りたか | AX が成功を返したのにテキストが入らなかったか |
|---|---|---|---|---|
| Slack | Electron のため AX 不可の可能性 → Pasteboard | 未実施 | 未実施 | 未実施 |
| Google Chrome | 要素により変動 | **実施（2026-08-14 / 2026-08-15）。アドレスバーは `ax`（`insert` 12〜34 ms / `total` 662〜891 ms）。ページ本体（`AXWebArea`）は書込不可で Pasteboard。**<br>**ページ内のフォーム欄（素の `<input>`）は `AXTextField` / settable=yes と答えながら書き込みが効かない → 検知して Pasteboard**（2026-08-15） | AX 経路のため該当せず | **フォーム欄で観測（2026-08-15）。R-4 の実観測 1 件目。**検知を §6.2 へ入れた |
| Xcode | AX 対応の見込み | 未実施 | 未実施 | 未実施 |
| Notion | Electron → Pasteboard | 未実施 | 未実施 | 未実施 |
| ターミナル / iTerm2 | Pasteboard | **未実施。ただし同型の Electron 製端末（cmux）で Pasteboard 経路を実施した（`insert` 316〜354 ms / `total` 1182〜1447 ms）** | **不足（実施時の値は 120 ms）。相手が ⌘V を処理する前に復元が走り、前のクリップボードの内容が貼られた → 既定を 300 ms へ引き上げた** | 該当せず（AX 経路ではない） |
| メモ / メール | AX 対応の見込み | **メモは実施（2026-08-14）。`ax`（`insert` 307 ms / `total` 1192 ms。相手が重い）。メールは未実施** | AX 経路のため該当せず | **観測されず** |

> **この表の「結果」列に入っている実測は、すべて 2026-08-14 / MacBook Pro Mac15,3 / M3 / macOS 26.5.2 のものである。**
> **300 ms へ引き上げた後の再実施は行っていない**（未実測）。上の「不足」は 120 ms のときの観測である。

あわせて次の 6 点を確認する。いずれも権限の無い機体では確かめられなかったもの。

1. **復元待ちの妥当性**（§6.3）。**現在の既定は 300 ms**（`PasteboardInserter.defaultRestoreDelay`）。**120 ms が不足であることは 2026-08-14 に実機で確かめた**が、**300 ms で足りることを確かめたのは cmux 1 本だけ**である。実測の 35 ms は WindowServer 経由の遅延を含まない下限にすぎない。貼り付けが空振りするアプリが 1 つでもあれば値をさらに上げる。**「120 ms は足りるか」はもう問わない——答えは出ている。**
2. **AX の無言失敗**（§6.2）。**2026-08-15 に Chrome のフォーム欄で観測したので、検知を実装した**（§6.2 の「無言失敗（R-4）の検知」）。当初の案（書き込み前に欄が空だった場合に限り読み戻す）は**採らなかった**——利用者が既に書いている欄でこそ起きるため、空を条件にすると実害のある場面を素通りする。採ったのは「キャレットが完全に動かない + 相手は読み戻しに応える + 書いた場所に無い」の 3 条件で、**判らない相手は成功として扱う**（二重挿入を作らない向き）。**残るアプリで最終列に該当が出たら、この規則で拾えているかを見る。**
...
6. **発話の長さと `refine` の実測を組にして記録する（V-25 の残り）。** 履歴の `refinedText` が無ければ打ち切られている。**打ち切り 750 ms は 3 秒の合成発話で決めた値であり、実用の発話長では足りない**（要件定義書 §2.8.4 (2)）。**NFR-P6b の目標 2 秒は 19 / 36 / 56 字の 3 点で既に実測済みなので（2026-08-15。§10「判ったこと 2」）、ここで取り直す必要は無い。欲しいのは 80 字以上の 1 点である**——3 点だけでは 121 字 ≒ 2.24 秒という外挿を確かめたことにならない。**打ち切り 3 秒は依然として推定値**で、その決め手（利用者が続きを打ち始めるまでの時間）は未実測である（V-29）。
...
| 5 | `TextInserter`（二段構え） | 二段構えが動く。**V-3 は AX API アクセスとキー送出の両権限が要るため §12-11 へ繰り延べ。手順は引き渡し手順書（`docs/04-handover-verification.md`）にある** |
| 6 | `HotkeyMonitor` | 判定と `CGEventTap` は完了。**V-4 はキーイベント監視の権限が要るため §12-11 へ繰り延べ。手順は引き渡し手順書（`docs/04-handover-verification.md`）にある** |
...
| V-3 | 主要アプリでの AX 挿入成否 | 実装 §12-11 | **一部実施（2026-08-14 / 実機。要件定義書 §2.8.5）。取得経路の誤りで AX が一度も使われていなかったことが判明し、修正後にメモと Chrome アドレスバーで `ax` になった**（Chrome: `insert` 12〜34 ms・`total` 662〜891 ms で **NFR-P6（現 NFR-P6a）を初達成**／メモ: `insert` 307 ms）。**R-4（AX が成功を返しながら何も入らない）は 2026-08-15 に Chrome のページ内フォーム欄（素の `<input>`）で観測した**——利用者のアンケートフォームで発話が 2 回消え、履歴には 2 件とも `ax` として残っていた。**検知して二段目へ落とす実装を入れ、実機で「フォーム欄は `pasteboard` で入る／アドレスバーは `ax` のまま」を確かめた**（§6.2）。残りのアプリ（メール / Slack / Notion / Xcode）は未実施。二段構えの実装と単体検査は完了。実挿入には AX API アクセス（`kTCCServiceAccessibility`）とキー送出（`kTCCServicePostEvent`）の**両方**が要り、無いと全アプリで `.inserted(.clipboardOnly)` になる。**CLI と手順は用意済み**（[引き渡し手順書](04-handover-verification.md)、記入先は §11.3 の表）。**Task 11 の実装者は権限を付与できないため実施していない。** |
| V-4 | 右 Option 押しっぱなしの副作用 | 実装 §12-11 | **未実施（利用者の権限付与待ち）**。判定ロジックと `CGEventTap` の実装・単体検査は完了。実キー入力の観測には `CGEvent.tapCreate` が通ること（`kTCCServiceListenEvent` / 入力監視）が要り、無いと 1 件も配送されない。**CLI と手順は用意済み**（[引き渡し手順書](04-handover-verification.md)、6 項目の表）。**あわせて実キーボードが左右のデバイスビット（`NX_DEVICERALTKEYMASK` 等）を立てることを確認する**（§2.3）。**Task 11 の実装者は権限を付与できないため実施していない。** |
...
| V-6 | `.nonactivatingPanel` がフォーカスを奪わないこと | 実装 §12-8 | **一部実測（2026-08-14 / MacBook Pro Mac15,3 / M3 / macOS 26.5.2 / 使い捨ての検証プログラム）。** **実測できたこと**: (a) `.borderless` + `.nonactivatingPanel` の `NSPanel` は `canBecomeKey` / `canBecomeMain` が既定で false、(b) `NSApp.run()` の**後**に `orderFrontRegardless()` すれば最前面アプリは変わらず `NSApp.keyWindow` / `mainWindow` も nil のまま、(c) `.accessory`（`LSUIElement` 相当）でも成立、(d) **`NSApp.run()` の前に出すとアプリが活性化して最前面が自分になる**（§7.2）。 **実装した HUD で測り直した（2026-08-15 / トラック C。§7.3 の「フォーカスを奪わないことの実測」）: 起動前・HUD 表示中・終了後を 0.25 秒ごとに観測して、最前面(layer0) が Ghost Voice になった回数は 2 回とも 0（32 回 / 28 回）。HUD の窓が layer 26 に現れることも外から観測した。** これは統合の申し送り 8（HUD を足した後のやり直し）にあたる。 **実バンドルでも測り直した（2026-08-15 / 署名済み `.app` を `/Applications` から起動 / `--hud-check`）**: `CGWindowListCopyWindowInfo` で HUD の窓が **layer 26 / alpha 1.0 / onscreen true / bounds (791,0,221,78)** に現れ、**切り欠きの帯に掛かる窓のうち 26 より前面のものは無かった**（メニューバー 24 / コントロールセンター項目 25）。窓の内側からも `isVisible true / level 26 / occlusionState に .visible` を確認した（§7.2）。 **なお未実測**: `CGEvent.post` による ⌘V 送出と同時に HUD が出ている状況、Space 切替・フルスクリーン（V-21）・Mission Control / Stage Manager 下 |
...
| V-19 | **`NSApp.run()` の下で `CGEventTap` のイベントが届くこと** | 実装 §12-8（**最初に潰す**） | **分解して一部を実測した（2026-08-14 / M3 / macOS 26.5.2 / 使い捨ての検証プログラム）。器の作り直しは不要。** 疑いの本体は「`NSApplication.run()` が `CFRunLoopGetMain()` を `.commonModes` で回すか」であってタップ固有ではないので、**タップを作らずに確かめた**——`CFMachPort` 由来の version 1 ソース（**タップと完全に同じ形**）/ version 0 ソース / タイマー / オブザーバの 4 系統、run() の前後どちらの登録でも、メニュー追跡中・モーダル中・`LSUIElement` の `.app` バンドル内でも発火。**陰性対照**（`.defaultMode` にだけ足したソース）は同条件で 0 回なので偽陽性ではない。配送遅延 p50 0.045 ms / 最大 5〜17 ms。**AppKit 下のコモン集合は既定・イベント追跡・モーダルパネルの 3 モードだけ**である（§7.2）。 **依然として未実施（利用者の実機待ち）: タップ固有の振る舞い。** `CGEvent.tapCreate` は入力監視の権限ダイアログを誘発するので一度も呼んでいない。**タップが実際にキーイベントを配送するか・`return nil` による抑止（ESC）が効くか・`.tapDisabledByTimeout` が出ないかは未実測である。** 届かなければ PTT がまったく動かない。手順は [引き渡し手順書](04-handover-verification.md) の「1. 最初にやること: 権限の付け直し」の 7 |
| V-20 | **notch の切り欠きそのものに描いた内容が見えるか** | 実装 §12-8 | **未実施（利用者の目視待ち）。** 座標（内蔵で x 791..1012 / y 1131..1169）は実測で取れているが、そこはカメラハウジングであり**描いても見えない可能性が高い（推測）**。**実装は「見えなくても成立する」形にしてある**——中身をすべて切り欠きより下へ置き、帯の黒は連続して見せるためだけに使う（§7.3）。したがって**この検証の結果で実装は変わらない**（見えるなら帯が繋がって見え、見えないなら中身だけが見える）。**2026-08-15 に判ったこと**: 窓そのものは実バンドルで確かに出る（帯を含む矩形 (791,0,221,78) が layer 26 に載り、内容も塗られている。§7.2）。**残っているのは「切り欠きの中に画素があるか」だけ**である。**手順**: [引き渡し手順書](04-handover-verification.md) の「段 7: HUD の目視」。**既存インスタンスを落としてから実行し、`pgrep` で PID が変わったことを必ず確かめること**——同じ `.app` が走っていると `open … --args` は黙って捨てられる（実測。手順書に手順あり）。`--hud-check` はマイクもキー監視も触らないので**権限が無くても実施できる** |
| V-21 | **`.canJoinAllSpaces` / `.fullScreenAuxiliary` が効くこと** | 実装 §12-8 | **未実施（利用者の実機待ち）。** Space を切り替え、他アプリをフルスクリーンにして目視する。Mission Control / Stage Manager 下も併せて見る。**指定してあること自体は検査で固定済み**（`HUDWindowContractTests`）だが、**効くかどうかは目視でしか判らない。** **手順**: [引き渡し手順書](04-handover-verification.md) の「段 7: HUD の目視」。`--hud-check=60` のように長めに指定して、その間に Space を切り替える。**既存インスタンスを落としてから実行すること**（`open … --args` は走っている `.app` には届かない。実測） |
| V-22 | **クラムシェル（内蔵が `NSScreen.screens` に無い）ときの表示先** | 実装 §12-8 | **未実施（利用者の実機待ち）。** 蓋を閉じたときの `NSScreen.screens` の中身を確かめる。**候補 (a)(b) のどちらを採るかは決着済み**——どちらも採らず「主ディスプレイのメニューバー直下」にした（基本設計書 §8.1.1。理由もそこにある）。**この検証で見るのは、決めた形が実際に成立するか**である: 内蔵が消えること、主ディスプレイへ出ること、起動時の診断が「内蔵ディスプレイが見つかりません」と言うこと。**あわせて外部ディスプレイを主にしたときの内蔵の `auxiliaryTop*Area` も見る**（§7.1 の未実測）。**手順**: [引き渡し手順書](04-handover-verification.md) の「段 7: HUD の目視」 |
| V-23 | **主要アプリで `kAXSelectedTextRange` が settable か。範囲の単位が UTF-16 か** | **§8.3 の実装より前**（V-3 の残作業と同じ回で取れる） | **未実施。** 対象はメモ / メール / Chrome アドレスバー / Chrome ページ内 / Slack / Notion / Xcode / ターミナル。`AXUIElementIsAttributeSettable` の実行時の答えだけが根拠になる（**SDK ヘッダの `Writable?` は当てにならない**——`kAXSelectedText` は `Writable? No` と書かれているのに、V-3 の実測では実際に書けている）。**ここが全滅なら差し替えは一度も成立しない**（挙動は現状と同じになるので、失うのは実装の工数だけ）。**権限を付与した利用者にしか実施できない** 実装は `AXStringForRange` の一致で自衛しているので、**外れても誤った位置には書かない**（全発話が「整形を待つ」分岐へ落ちるだけ）。手順は引き渡し手順書（`docs/04-handover-verification.md`）の段 5 |
...
| V-25 | **発話長と整形所要の関係**（20 / 40 / 60 / 80 / 120 字 × 各 10 回。低負荷と負荷下の 2 条件） | **§8.3 の実装より前**（**最初に取るとよい**） | **主要部を実測した（2026-08-15 / 配線トラック。§10 の「(a)/(b) の実分布と、発話長と整形所要の関係」）: 19 字 343〜378 ms / 36 字 652〜716 ms / 56 字 1016〜1067 ms、傾き 約 18.5 ms/字。NFR-P6b の目標 2 秒の根拠だった外挿はこれで支持された。** **【2026-08-15 の実運用の切り分けでこの直線は否定された（要件定義書 §2.8.9）】**——3 点は発話ごとに 3〜10 秒の音声再生を挟んで測っており、**各点がモデルの冷え直しの費用を含んでいた。**温まったデーモンで測り直すと **365 ms + 5.26 ms/字**（利用者の実発話 9 種 × 6 回 / 1〜161 字 / 連続投入）で、**750 ms を割るのは約 73 字**である。**「18.5 ms/字」を引用しないこと。** 残るのは 80 字以上の帯と、合成音声ではなく**肉声**での確認である。以下は実測前の記述: **NFR-P6b の目標 2 秒 / 打ち切り 3 秒は推定値であり、直接の根拠は「40 字以上の 8 件が 750 ms を超えた」（下限しか判らない）と `refine 791 ms` の 1 件しかない**（要件定義書 §2.8.4 (2)）。結果次第では (b) 分岐の打ち切り値も変わる。**マイクと Apple Intelligence があれば取れる**（AX の権限は要らない） |
...
| V-27 | **競合させても必ず差し替えを断念できるか**（別アプリへ移動 / 別入力欄へ移動 / カーソル移動 / 記録範囲の前・中・後への追加入力） | **§8.3 の実装直後** | **未実施。** 一致条件を失った場合に**内容変更が 0 件**であること、生テキストが回収可能であることを見る。**手順 2 と 4 の間の競合窓は原理的に閉じられない**（AX に compare-and-swap が無い）ので、ここは「必ず閉じる」ではなく「どれだけ狭いか」を測る検証である **あわせて `CFEqual` による AX 要素の同一性が時間を跨いで期待どおりかを見る**（§6.5 の C-4。外れると「別の欄だ」と判定して中止に倒れるので**誤った欄へは書かない**。症状は「差し替えが一度も効かない」）。手順は引き渡し手順書（`docs/04-handover-verification.md`）の段 5 |
...
| V-36 | **差し替えが PTT の押下と重なったときの M1a** | §8.3 の実装後 | **代役の欄に対して対処の前後を実測済み**（2026-08-15。詳細設計書 §10.1）。**差し替えは actor を手放して走る**ようになり（`runOffActor`。重なりは世代の錠が塞ぐ）、押下 → 録音開始は **AX の往復コストに依存しなくなった**——1 往復 10 ms を注入した相手で **最大 164 ms（低負荷）/ 170 ms（負荷下）→ 対処後 3.1 / 3.0 ms**。注入 0 ms では前後とも 3 ms 未満。**対処前は待たされる量がほぼ `12 × AX 1 往復`** で、AX の上限（1 往復 0.5 秒）に当たる相手では**最大約 6 秒 actor が塞がり、その間の PTT が処理されず発話が丸ごと落ちた**（最終レビュー 視点3 の指摘 2）。**残るのは実アプリでの 1 往復のコスト**だが、待たされる量の判断には要らない（V-28 で挿入の所要として測る） |
...
| V-34 | **発話の途中の終了要求（⌘Q / SIGTERM）で発話が失われないか（`.app` 版）** | V-19 の後 | **一部実施（2026-08-15 / M3 / macOS 26.5.2）。** `applicationShouldTerminate` は `.terminateLater` を返し、`GhostVoiceCore.Shutdown.perform` が待機へ戻るまで待ってからホットキーを止める（**CLI と同じ 1 つの実装**。門を持たない分、待つ根拠は `isBusy` だけ）。**この項目が「実発話でしか通らない」とされていたため実バンドルで一度も測られず、`SIGTERM` でも `pkill` でも終了しない `.app` が出荷された**（`bug-term`。受け口が `DispatchSource(queue: .main)` で、メインキュー排出中に `.terminateLater` の入れ子のランループへ入り、返事を出す `Task` が永久に走らなかった）。**受け口を専用キューへ移し、メインへは `MainRunLoopHop`（`CFRunLoopPerformBlock`）で渡すよう直した。** 実測: SIGTERM / SIGINT / `osascript` の quit いずれも効き、発話を抱えていなければ 0.13〜0.23 秒、3 秒抱えていれば 3.15〜3.34 秒で終了する（`--shutdown-check` で測る。手順は引き渡し検証 §段 2）。**残り: 実発話での 1 回と、⌘Q の実打鍵**（`CGEvent.post` を使えないため未実測。経路は `osascript` と同じ）。**2026-08-15 に実発話で 1 回通り、2 件の欠陥が出た。**(1) **終了待ちの案内が `.app` では unified log にしか出ず、画面に何も出なかった**——利用者は「キーを離してください」を一度も見ないまま猶予 10 秒を使い切った。→ HUD へ出すようにした（§7.4「終了待ちを HUD に出す」。目視は `--hud-check`）。(2) **猶予切れで打ち切った発話が、欄にもクリップボードにも履歴にも残らなかった**——`ESC` による中断は履歴へ残すのに、終了の打ち切りだけ穴が空いていた。→ 打ち切るときにそこまでのテキストを履歴へ残す（§4「終了の猶予切れで打ち切った発話を捨てない」）。**どちらも代役の検査は緑のままだった。** 前者は `--hud-check` の一覧と実バンドルの目視で、後者は `ShutdownSalvageTests`（代役の監視器でイベント列を終端し、この経路を決定的に踏む）で埋めた。文言の `10.0 seconds` は `JapaneseDuration` を通して「10 秒」にした。**2026-08-15、修正後の実バンドルで利用者が同じ手順を再実行し、成立を確認した**（`(sleep 5; kill -TERM …)` を仕掛けてから PTT を押して発話。**HUD に「PTT キーを離してください（残り N 秒）」が出た**と利用者が確認し、その案内を見て離せたため、発話は打ち切られず `pasteboard` 経路で挿入され履歴にも残った。**前回はまったく同じ手順で発話が丸ごと消えている。猶予は 10 秒のまま変えていないので、差を作ったのは「見えるかどうか」だけである**——案内が画面に出るかどうかが、発話を失うか否かを分けた実測例）。 |
...
| V-40 | **ディスプレイの抜き差しで HUD が付いていくか** | HUD の実機確認時 | **未実施。** `NSApplication.didChangeScreenParametersNotification` を購読して再配置する実装は入れてあるが、**通知が実際に来ることを確かめていない**（抜き差しの操作が要る）。来なければ HUD が古い座標に出続けるだけで、**挿入は壊れない。** 外部ディスプレイを抜き差しし、内蔵の切り欠きに出続けることを見る。**2026-08-15 追記**: 再配置（`HUDPanel.relocate`）は `orderFront` を呼ばないので、**表示中に構成が変わると矩形だけが移る**。出し入れの行（`[HUD] 窓を出しました: …`）で矩形を追えるようにした |
...
| V-44 | **`NSApp.hide(nil)` で前面が確実に戻るか** | 設定／履歴の窓を配線した後 | **実測して達成（2026-08-15 / 同上）。戻る。** `--window-check` で設定 → 履歴の順に開閉し、`kCGWindowLayer == 0` の最前面 pid を 4 ms 間隔で追った。**窓を出していない区間では 1 度も奪っていない**（HUD は layer 26）。**窓を開いた区間では意図どおり Ghost Voice が最前面になり、閉じると元のアプリ（起動前と同じ pid）へ戻った。** `--hud-check`（HUD を実際に表示した状態）でも **1,488 回の観測で奪取 0 回。** **ただし戻るまでの遅れがある**（V-43）。 **【母数の訂正 / 2026-08-15】** この実行の**総観測数は 2,451 回だが、うち 1,154 回は窓を開いていた区間（＝意図した奪取）である。** したがって「窓なし区間で 2,451 回」は誤りで、**多く見ても 1,297 回**である。**`--window-check` の 2,451 と `--hud-check` の 1,488 を足して「3,939 回で奪取 0」と読んではならない。** **残る未実測: 実バンドル（`Ghost Voice.app`）での確認**（素の実行ファイルで測った）、**Space 切替・フルスクリーン・Stage Manager・別機体**、および **4 ms 未満の一過性の奪取**（観測間隔より短い事象は原理的に見えない） |
| V-45 | **実運用で整形が効いているか（利用者が自分で確かめる）** | 引き渡し後、実機で 5 分 | **未実施（利用者が実施する）。手順は引き渡し手順書 §段 6 の V-45。** **背景**: 2026-08-15 の実運用で 10 発話中 2 件しか整形されず、しかもその 2 件はモデルが入力をそのまま返した回だった（要件定義書 §2.8.9）。**原因は打ち切りで、その打ち切りの主因は発話長ではなくモデルの冷えだった**（間隔が空いた 1 発目 0.7〜2.1 秒 / 直後の 2 発目 0.33〜0.49 秒。§5.2）。**押下の時点で暖め直す**ようにしたが、**実機・肉声・実アプリでの効果は未確認である。**見るのは 3 点——①**1 分以上あけた 20 字**で `refinedText` が入るか（入らなければ暖機が効いていない）、②**5 秒以内に続けた 20 字**でも入るか、③**80 字以上**は入らないことがある（設計どおり。温まっていても 365 + 5.26×字 で 750 ms を割る）。**辞書を入れた状態でも見ること**（辞書が置換を強制していた欠陥は直したが、短い発話では辞書が整形の成功率を下げる実測がある） |
| **V-46** | **アイドル 30 秒でマイクのオレンジ点が消え、押下で戻ること** | 実装 §12-3（本改訂と同時） | **成立（2026-08-16 / 実機 / 署名済み `.app` を `/Applications` から 16 時間常駐させた状態 / M3 / macOS 26.5.2）。利用者が「オレンジ点が消えていた」ことを目視で確認した。** あわせて `coreaudiod` の CPU で停止の時刻を挟み込んだ——16:25:28 の 1 発話（「お疲れ様です」）の後、**14.6〜19.4% が 7 窓（約 35 秒）続き、16:26:04〜16:26:09 のあいだに 3% 台へ落ちた**（5 秒窓）。**停止は発話から約 36〜41 秒後**で、30 秒の起点が「最後の発話＝セッションの終わり（認識・整形の後）」であることと整合する。**5 秒刻みなのでこれ以上の精度は無い。** **押下で戻ることは、同じ発話で CPU が 3% 台から 17.20% へ立ち上がったことで裏づけた**（エンジン起動＝点灯は設計書 §2.1 の実測）**が、戻った点そのものは目視していない。** **頭欠けも確かめた（2026-08-16 / 利用者の実発話）——欠けていなかった。** ただし**これは 1 回の観測である**。起床の 67.3 ms のあいだ音が存在しないことは変わらないので、**「原理的に欠けうるが、この 1 回では出なかった」が正確な言い方である。** **計測の注意は下の「§13 補記」を見ること。** 手順は引き渡し手順書の V-46 |
| **V-47** | **待機中の `coreaudiod` の CPU が停止時の水準へ落ちること**（電力） | 実装 §12-3（本改訂と同時） | **達成（2026-08-16 / 実機 / 同上）。待機中 2.6〜4.6%、発話直後の起きている区間 14.6〜20.8%。** 設計時の値（停止 4.3〜4.8% / 起動 19.6〜20.3%。設計書 2026-08-15 §2.3）と同じ水準であり、**待機中はむしろ設計時の下限より低い。** 16 時間アイドル後の基準値は **3.55%**（20 秒窓）。**常時ウォーム時代の 19.6〜20.3% は、発話直後の約 35 秒間だけになった。** **自プロセスを見ても判らない**——差は 0.11% 対 0.03% で誤差であり、費用は `coreaudiod` に立つ。**計測の注意は下の「§13 補記」を見ること。** 手順は引き渡し手順書の V-47 |

### §13 補記: `coreaudiod` で寝ているかを見るときの落とし穴（2026-08-16）

**単発の跳ねと、持続する台地を混同してはならない。**

**一度も喋らない 3 分間（36 窓 / 5 秒窓）を測ったところ、12〜14% の窓が単発で現れた**
（12% 超が 2 窓、8〜12% を入れるとさらに数窓）。**これは Ghost Voice ではない**——
この機体には Chrome / Slack / Microsoft Teams / Zoom / Cisco の仮想オーディオドライバが常駐しており、
**そのどれかが短時間デバイスを触っている。** いずれも 1 窓（5 秒）で終わり、続かない。

**Ghost Voice のエンジンが起きている署名は「14〜20% が連続して 6〜7 窓つづく台地」である。**
実測では発話後に 7 窓つづき、そこから 3% 台へ落ちた。

**1 点だけ測って判定してはならない。** 「20 秒窓で 1 回」という当初の手順（上の V-47 のコマンド）は、
**跳ねを踏むと寝ているのに起きていると誤判定する。** 時系列で 5 秒ごとに刻み、
**台地になっているかを見ること。**

**この汚染は設計時にも踏んでいる**——設計書 §2 の但し書き（「Ghost Voice.app を起動したまま測って
最初の計測が無効になった」）と同じ型で、**測定対象以外がデバイスを触りうるという同じ事実の別の面である。**
...
| **`NSApp.hide(nil)` で前面が確実に戻るか** | **戻る。** `--window-check` で 4 ms 間隔の観測。窓を出していない区間は奪取 0 回（**総観測 2,451 回のうち 1,154 回は窓を開いた区間なので、窓なし区間の母数は多くても 1,297 回**）、閉じた後は起動前と同じアプリへ戻った | **V-44** |
