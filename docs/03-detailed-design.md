...
`isModifierDown` はデバイスビットが引ければ `binding.modifiers` を参照しない。したがって `HotkeyBinding(keyCode: 0x3D, modifiers: [.option, .shift])`（⇧ + 右 Option）を設定しても、**右 Option 単独で発火する。** モデル側にこれを禁じる仕組みは無いので、**設定 UI（Task 11）は修飾キー単独のバインドに追加の修飾キーを付けさせないこと。**
...
→ **マイクを使うには `.app` バンドル + `Info.plist`（`NSMicrophoneUsageDescription`）+ 署名が前提条件である。**
`swift run` や素の実行ファイルでは永久にマイクを使えない。Task 11（CLI と一気通貫）はこれを踏む。
...
**ただし逃げ道として記録しておく。** Task 10 が M5 を実測して 1000 ms を割り込むなら、
...
| 計測 ID | 区間 | 目標 |
|---|---|---|
| `M1a` | キー押下 → **タップ武装**（取りこぼしが止まる時点） | 50 ms（NFR-P1）。**実測 中央値 0.088 ms（アイドル）／ 0.118 ms（負荷下）、最大 14.0 ms**。うち `begin()` は実測 1.2〜1.4 ms。**達成（V-9 実施済み）** |
...
| `M5` | キー解放 → 挿入完了（M2+M3+M4） | **1000 ms（NFR-P6）。実測 中央値 398 ms（低負荷）／ 411 ms（負荷下）、p90 419 / 819 ms。全条件で 10/10 が達成（Task 10 実施済み）。ただし M4 の留保 3 件に注意（下記）** |
...
**NFR-P6（1000 ms）はどの条件でも 10/10 で達成している。** 問題はそこではない。
捨てられたバッファも全条件で 0 件だった。
...
2. **AX の無言失敗**（§6.2）。最終列に 1 件でも該当があれば、**二重挿入を避ける形**で検知を設計する。案: 書き込み前に対象が空だった場合に限り読み戻して検証する（空なら「アプリによる変換」と紛れないため誤検知しにくい）。設計は Task 11 で行う。
...
| 7 | `DictationSession`（状態機械） | **状態機械と計測は完了（Task 10）。M5 を 2 条件で実測し、整形の既定タイムアウトを 750 ms へ引き上げた（§10）。** CLI での一気通貫は Task 11 |
| 8 | `NotchHUD` | **V-5 / V-6 を実施する** |
| 9 | 設定 UI・権限フロー・履歴 UI | FR-7〜FR-11 が満たされる |
| 10 | 性能計測と調整 | **M5 は実測済みで 1000 ms 以内（中央値 423 / 551 ms、最大 550 / 866 ms。§10）。ただし挿入は `.clipboardOnly` 経路での計測であり、⌘V を含む確定は V-3 待ち。V-7（メモリ）は未確認** |
...
| V-9 | **実マイクでの NFR-P1**（M1a のタップ武装、M1b の初回バッファ到達、タップ長の実際値） | 実装 §12-3 | **完了**。M1a 中央値 0.088〜0.118 ms（**達成**）、M1b 中央値 106.5〜106.7 ms（50 ms 不可）、タップ長 4800 フレーム＝100.0 ms（`bufferSize` 64〜4800 のどれでも同じ）、入力 48000 Hz / 1 ch、`prepare()` 327〜456 ms。**100 ms の下限は HAL（512 フレーム）ではなく `AVAudioEngine` のタップ実装**（§3.5 / §3.6） |
...
| V-12 | **キー解放後に届く確定（`.final`）が 1 回とは限らないこと** | 実装 §12-11 | **未実施。** `DictationSession` は「キー解放以降の最初の `.final`」で先へ進む（§10 の M2 の定義そのまま）。計測した 20 発話（3 秒 / 日本語）ではいずれも解放後の確定は 1 回で、取りこぼしは観測していない。**長い発話や区切りの多い発話で解放後に 2 回目の確定が出るなら、その分の文字が落ちる。** 実発話での確認が要る |
