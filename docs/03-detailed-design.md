...
| 測ったこと | 実測値 |
|---|---|
| `CGWindowListCopyWindowInfo` に現れる HUD の窓 | `layer 26 / alpha 1.0 / onscreen true / bounds (791, 0, 221, 78)`（CG 座標＝左上原点） |
...
既定の秒数は 2 つの筋書きの総和（約 12.1 秒）を上回るよう **16 秒**にしてある（`HUDRehearsalTests` が総和と比べて固定）。
