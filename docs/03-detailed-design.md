...
| `M1b` | キー押下 → 最初のバッファ**到達** | **実測 中央値 106.7 ms（アイドル）／ 106.5 ms（負荷下）、最小 102.9 / 最大 139.8。50 ms では届かない。** タップの粒度 100 ms が下限（§3.5）。M5 の内訳として扱う |
...
| V-9 | **実マイクでの NFR-P1**（M1a のタップ武装、M1b の初回バッファ到達、タップ長の実際値） | 実装 §12-3 | **完了**。M1a 中央値 0.088〜0.118 ms（**達成**）、M1b 中央値 106.5〜106.7 ms（50 ms 不可）、タップ長 4800 フレーム＝100.0 ms、入力 48000 Hz / 1 ch、`prepare()` 327〜456 ms（§10） |
