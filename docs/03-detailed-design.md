...
### 許可の対象は実行ファイルではなく責任プロセスである（実測 / Task 11）
...
| 入口 | 何をするか | ダイアログ |
|---|---|---|
| `ghost-voice --check` | 5 項目を照会して表に出す。足りなければ終了コード 1 | **出さない** |
...
ターミナルアプリを再起動してから各アプリを回る。所要は準備 5 分・V-4 が 10 分・V-3 が 20 分。
...
| 11 | **CLI と一気通貫**（`ghost-voice`） | **完了（Task 11）。** 起動・権限案内・表示・終了の待ち合わせが動く。`--check` / `--request-permissions` / `--mic-check` を用意した。**V-3 / V-4 は権限の付与が要るため利用者が実施する**（README の手順） |
