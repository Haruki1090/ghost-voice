...
- **(a) では打ち切りではない。** M3 は 969〜1152 ms で**締め切り（3 秒）の内側に収まっている**のに、
  **整形結果は 10/10 で捨てられた。** 捨てたのは `RefinementGuard`（§5.5.1）である——
  出力が入力の変換になっていない（残存率が閾値を割った）と判定された。

> **これは新しい観測である**（2026-08-15）。**打ち切りを延ばしても解けない種類の未反映**であり、
> フェーズ 1 の「40 字以上が全部打ち切られた」（要件定義書 §2.8.4 (2)）とは原因が違う。
> **ただし観測は合成音声 1 本 56 字ぶんしかない。** 閾値をいじる根拠には足りない
> （§5.5.1 の注記「正当な整形を落とす報告が出たら、**閾値ではなく指標そのものを見直すこと**」）。
> **検証項目 V-37 として登録する。**
...
| V-19 | **`NSApp.run()` の下で `CGEventTap` のイベントが届くこと** | 実装 §12-8（**最初に潰す**） | **未実施。** `CGEventTapHotkeyMonitor` はソースを `CFRunLoopGetMain()` の `.commonModes` へ足すので届くはずだが（§7.2）、**これは推測である。** 届かなければ PTT がまったく動かないので、フェーズ 2 の最初に確かめる **フェーズ 1 の CLI は `CFRunLoopRun()` を自前で回していたため、`NSApp.run()` との組み合わせは 1 度も動いていない。** 手順は [README](../README.md) の「フェーズ 2: `Ghost Voice.app` への移行」 |
...
| V-36 | **差し替えが PTT の押下と重なったときの M1a** | §8.3 の実装後 | **未実施。** 差し替えは actor の上で**同期に**走る（世代の照合と実際の書き込みの間に窓を作らないため）。**その数 ms〜数十 ms のあいだに PTT が押されると `startRecording` が待たされ、NFR-P1 の 50 ms を食う。** 所要そのものは V-27 の `M7` と同じ量なので、そちらと同じ回で取れる |
| V-37 | **長い発話で `RefinementGuard` が正当な整形を捨てていないか** | §5.5.1 の見直しの前 | **未実施。** **2026-08-15 の実測で、56 字の発話は整形が締め切り（3 秒）の内側 969〜1152 ms で完了しているのに 10/10 で捨てられた**（残存率が閾値 0.6 を割った。§10 の「判ったこと 3」）。**打ち切りを延ばしても解けない種類の未反映である。** 観測は合成音声 1 本 56 字ぶんしかないので、**閾値をいじる根拠には足りない**——§5.5.1 の注記どおり、正当な整形を落とす報告が肉声でも出るなら**閾値ではなく指標そのものを見直す** |
