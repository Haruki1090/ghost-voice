...
| **V-46** | **アイドル 30 秒でマイクのオレンジ点が消え、押下で戻ること** | 実装 §12-3（本改訂と同時） | **未実施（利用者の目視待ち）。** 常駐させて最後の発話から 30 秒待ち、メニューバーを目視する。その後 PTT を押して点が戻ることも見る。**機構側は自動検査で固定済み**（`sleepStopsEngine` / `reconfigurationDoesNotWakeSleepingEngine` ほか）だが、**点そのものが消えるかは目視でしか判らない**——`kAudioDevicePropertyDeviceIsRunningSomewhere` が false になることは設計時に実測済み（設計書 2026-08-15 §2.1）。手順は引き渡し手順書の V-46 |
| **V-47** | **待機中の `coreaudiod` の CPU が停止時の水準へ落ちること**（電力） | 実装 §12-3（本改訂と同時） | **未実施（利用者の実機待ち）。** 常駐させて 30 秒待った後、`ps -Ao time,comm` で `coreaudiod` の累積 CPU 時間を 20 秒あけて 2 回読み、差分から % を出す。**4.3〜4.8% の水準まで落ちれば良い**（常時ウォーム時は 19.6〜20.3%。設計書 2026-08-15 §2.3）。**自プロセスを見ても判らない**——差は 0.11% 対 0.03% で誤差であり、費用は `coreaudiod` に立つ。手順は引き渡し手順書の V-47 |
