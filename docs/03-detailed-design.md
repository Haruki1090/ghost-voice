...
> **この状態から抜ける手段が、現状 ESC（`.cancelled`）とタップ無効化通知しか無い。** 時間による安全弁が無い。**Task 10 の状態機械に最大録音時間を持たせること**（`DictationSession` が `.released` を待ち続ける設計になると、この経路でハングする）。`HotkeyMonitor` 側に置かないのは、適切な上限が録音の要件（NFR）側の値であり、監視器はキーの状態しか知らないためである。
