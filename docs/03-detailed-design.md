...
> (b) では形が変わらないので、**既定値 750 ms も変えない**（余裕は 1 ms しか無い。引き直しは配線トラックで行う）。
