...
public struct PasteboardInserter: PrimaryInserting, ClipboardLeaving {
    ...
    ///
    /// **V-3 で 120 ms は不足と判明した（2026-08-14 / 実機）。既定を 300 ms へ引き上げた。**
    ///
    /// 上の留保 2 がそのまま現実になった。Electron 製の端末アプリ（cmux）へ挿入すると、
    /// **相手が ⌘V を処理する前に復元が走り、戻された「前のクリップボードの内容」が
    /// 貼られた。** 履歴には正しい発話が `pasteboard` で成功として記録されるので、
    /// **「成功と記録されるのに、そこに違うテキストがある」**——このプロジェクトが
    /// 繰り返し潰してきた形である。連続発話で悪化した（前の発話の復元が、次の発話の
    /// 書き込みと ⌘V の間へ割り込むため）。`--paste-restore-delay-ms 300` で解消した。
    ///
    /// **300 ms が上限である保証は無い。** 確かめたのは「120 では壊れ、300 では壊れない」
    /// までで、間の値も、より重いアプリも測っていない。**壊れ方がデータの取り違えなので、
    /// 速さより余裕を優先している。**
    ///
    /// なお、この待ち時間は NFR-P5（テキスト挿入 50 ms 以内）には数えない。
    /// 挿入はテキストが貼り付いた時点で完了しており、復元はその後始末である。
    /// **ただし `Metrics` の M4 は `insert(_:)` の実時間を測るので、この待ちが丸ごと乗る**
    /// （実測: 復元待ち 120 ms のとき M4 は 170 ms、300 ms のとき 324〜342 ms）。
    /// **復元を `insert(_:)` の復帰後へ移せば、この待ちはクリティカルパスから外れる。**
    /// フェーズ 2 の課題（詳細設計書 §6.3）。
